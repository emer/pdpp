...
static ivCoord* mlx, *mly;

/*****************************************************************************/

PointObj::PointObj (ivCoord x, ivCoord y) { _x = x; _y = y; }
...
LineObj::LineObj (ivCoord x0, ivCoord y0, ivCoord x1, ivCoord y1) {
...
BoxObj::BoxObj (ivCoord x0, ivCoord y0, ivCoord x1, ivCoord y1) {
...
bool BoxObj::operator== (BoxObj& box) {
    float tol = 0.0001;
    return ( 
        osMath::equal(_left, box._left, tol) && 
        osMath::equal(_right, box._right, tol) &&
        osMath::equal(_bottom, box._bottom, tol) && 
        osMath::equal(_top, box._top, tol)
    );
}
...
bool BoxObj::Intersects (LineObj& l) {
    ivCoord x1 = min(l._p1._x, l._p2._x);
    ivCoord x2 = max(l._p1._x, l._p2._x);
    ivCoord y1 = min(l._p1._y, l._p2._y);
    ivCoord y2 = max(l._p1._y, l._p2._y);
...
MultiLineObj::MultiLineObj (ivCoord* x, ivCoord* y, int count) {
...
void MultiLineObj::GrowBuf () {
    ...
    if (mlsize == 0) {
        mlsize = NUMPOINTS;
	mlx = new ivCoord[NUMPOINTS];
	mly = new ivCoord[NUMPOINTS];
    } else {
	newsize = mlsize * 2;
	newx = new ivCoord[newsize];
	newy = new ivCoord[newsize];
	osMemory::copy(mlx, newx, newsize * sizeof(ivCoord));
	osMemory::copy(mly, newy, newsize * sizeof(ivCoord));
	delete mlx;
	delete mly;
	mlx = newx;
	mly = newy;
...
void MultiLineObj::SplineToMultiLine (ivCoord* cpx, ivCoord* cpy, int cpcount) {
    register int cpi;
...
void MultiLineObj::ClosedSplineToPolygon (ivCoord* cpx, ivCoord* cpy, int cpcount){
    register int cpi;
...
static int LowestLeft (ivCoord* x, ivCoord* y, int count) {
    register int i;
    int lowestLeft = 0;
    ivCoord lx = *x;
    ivCoord ly = *y;
...
void FillPolygonObj::Normalize () {
	...
	_normCount = limit + 2;
	_normx = new ivCoord[_normCount];
	_normy = new ivCoord[_normCount];
  ...
  for (int i = 0; i < _normCount - 2; ++i) {
      ...
      if (next_y_sign != cur_y_sign) {
          ...
          if (cur_x_sign && next_x_sign) {
              ...
              ivCoord dx = l._p2._x - l._p1._x;
              ivCoord dy = l._p2._y - l._p1._y;
