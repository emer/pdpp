...
compute_output(s) int s;{
  ...
  for (i = 0; i < nunits; i++) { /* to this unit */
    ...
    net = bias[i];
    sender = sact[s-1]+first_weight_to[i];
...
compute_output_error(s) int s; {
  ...
  for (i = 0; i < nunits; i++) {
    if (starg[s][i] >= min) {	/* We care about this one */
      t = starg[s][i] - sact[s][i];
      sss[s] += t*t;
...
bperror(s) int s; {
  register int i,j,first,num,end;
  FLOAT *wt, *sender;
  FLOAT del;
  FLOAT range = max - min;
  ...
  for (i = 0; i < nunits; i++) {
    del = sdelta[s][i];
    wt  = weight[i];
    end = first_weight_to[i] + num_weights_to[i];
    for (j = first_weight_to[i] ; j < end ; j++, wt++){
...
decay_weights() {
  ...
  if (wzero <= (FLOAT) 0.0) {	/* exponential weight decay */
    for (i = 0; i < nunits; i++) {
      wcost  += bias[i]*bias[i];
      bed[i] -= wdrate*bias[i];
      wt = weight[i]; e = num_weights_to[i];
      we = wed[i];
      for (j = 0; j < e; j++) {
	wcost += wt[j]*wt[j];
	we[j] -= wdrate*wt[j];
    ...
    for (i = 0; i < nunits; i++) {
      ...
      for (j = 0; j < e; j++) {
	/* sw = wt[j]/wzero; */
	sw = wt[j]*wt[j]*one_over_wzero_sq ;
	wcost += sw / ((FLOAT) 1.0 + sw) ;
	we[j] -= wdrate*melt(sw); /* also here (dcp) */
      }
    }
  }
  wcost *= wdrate ;
...
change_weights() {
  register int    i,e,j;
  register FLOAT *wt, *we, *dwt, *epi, *wi, *end;
  FLOAT twdr,sw;
...
symmetrize_weds() {
  ...
  for (i = 0; i < nunits; i++) {
    for (j = i; j < nunits; j++) {
      if ( (wj = (j - first_weight_to[i])) < 0) continue; 
      if (wj >= num_weights_to[i]) continue;
      ijval = wed[i][wj];
      if ( (wi = i - first_weight_to[j]) < 0) continue;
      if ( wi < num_weights_to[j] ) {
	jival = wed[j][wi];
	wed[i][wj] = wed[j][wi] = ijval + jival;
      }
    }
  }
}
...
setup_pattern() {
  ...
  for(s = 0; s < nsteps+1; s++) {
    for (u = 0; u < nunits; u++) {
      ...
      ssignet[s][u] = sact[s][u] = (min + (max - min)/2.0);
  ...
  for (u = 0; u < nunits; u++) {
    if (sclamp[0][u] >= min) {
      ...
      ssignet[0][u] = sact[0][u] = (min + (max - min)/2.0);
    }
  }
  if (setp->nalts > 1) {
    rval = rnd();
