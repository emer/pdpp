...
// 									      //
// IN NO EVENT SHALL CAREGIE MELLON UNIVERSITY BE LIABLE FOR ANY SPECIAL,    //
...
public:
  ...
  bool		cyc0_vm_eq;	// #DEF_false in first cycle, compute the vm at equilibirium potential given current inputs (for layers that receive primarily from fixed inputs)
...
public:
  ...
  virtual float	Compute_IThreshAll(LeabraUnit* u, LeabraLayer* lay, LeabraTrial* trl);
  // compute inhibitory value that would place unit directly at threshold, using all currents
  virtual float	Compute_IThreshNoA(LeabraUnit* u, LeabraLayer* lay, LeabraTrial* trl);
  // compute inhibitory value that would place unit directly at threshold, excluding gc.a current
  virtual float	Compute_IThreshNoH(LeabraUnit* u, LeabraLayer* lay, LeabraTrial* trl);
  // compute inhibitory value that would place unit directly at threshold, excluding gc.a current
...
public:
  ...
  float		pat_q;		// #DETAIL #DEF_0.5 threshold for pat_k based activity level: add to k if ext > pat_q
...
class KwtaTieBreak : public taBase {
  // #INLINE ##NO_TOKENS #NO_UPDATE_AFTER break ties where all the units have similar netinputs and thus none get activated.  this lowers the inhibition so that all get active to some extent
public:
...
class LayNetRescaleSpec : public taBase {
  // #INLINE ##NO_TOKENS #NO_UPDATE_AFTER global rescale of layer netinputs to prevent blowup
public:
  bool		on;		// whether to apply layer netinput rescaling
  float		max_net; 	// #CONDEDIT_ON_on:true #DEF_0.8 target maximum netinput value
...
public:
  ...
  virtual float	Compute_PVe(LeabraLayer* lay, LeabraTrial* trl, bool& actual_er_avail, bool& pv_detected);
...
class MatrixRndGoSpec : public taBase {
  // ##INLINE ##NO_TOKENS #NO_UPDATE_AFTER misc random go specifications (unconditional, nogo)
public:
...
class MatrixErrRndGoSpec : public taBase {
  // ##INLINE ##NO_TOKENS #NO_UPDATE_AFTER matrix random Go firing to encourage exploration when (a series of) errors occur: a stripe is chosen from a softmax over the snrthal netinputs (closer to firing chosen with higher probability)
public:
...
class MatrixAvgDaRndGoSpec : public taBase {
  // ##INLINE ##NO_TOKENS #NO_UPDATE_AFTER matrix random Go firing to encourage exploration for non-productive stripes based on softmax of avg_go_da for that stripe (matrix_u->misc_1)
public:
...
class PFCGateSpec : public taBase {
  // ##INLINE ##NO_TOKENS #NO_UPDATE_AFTER gating specifications for basal ganglia gating of PFC maintenance layer
public:
  enum	GateSignal {
...
class PFCOutGateSpec : public taBase {
  // ##INLINE ##NO_TOKENS #NO_UPDATE_AFTER specifications for pfc output gating
public:
...
class PFCOutLayerSpec : public LeabraLayerSpec {
