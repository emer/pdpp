...
public:
  ...
  enum SoftBound {
    ...
    SB_ERR,			// soft-bound the error term
  };
  ...
  inline void		C_UpdateWeights(LeabraCon_Group* cg, LeabraCon* cn,
...
inline float LeabraConSpec::C_Compute_Hebb(LeabraCon* cn, float ru_act, float su_act,
					   float, float inc_cor, float dec_cor)
{
  return ru_act * (inc_cor * su_act * (1.0f - cn->wt_lin) -
		   dec_cor * (1.0f - su_act) * cn->wt_lin);
}
...
inline void LeabraConSpec::Compute_dWt(Con_Group* cg, Unit* ru) {
  ...
  float savg = Compute_SAvgCor(lcg, inc_cor, dec_cor);
...
inline void LeabraConSpec::C_UpdateWeights(LeabraCon_Group* cg, LeabraCon* cn,
...
inline void LeabraConSpec::UpdateWeights(Con_Group* cg, Unit* ru) {
  LeabraCon_Group* lcg = (LeabraCon_Group*)cg;
  LeabraUnit* lru = (LeabraUnit*)ru;
  CON_GROUP_LOOP(cg, C_UpdateWeights(lcg, (LeabraCon*)cg->Cn(i),
				     (LeabraUnit*)ru, (LeabraUnit*)cg->Un(i)));
  UpdtWtMinMax(lcg, lru);
  AdaptWtGain(lcg, lru);
}
