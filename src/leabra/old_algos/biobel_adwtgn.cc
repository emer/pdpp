...
void BioBelCon_Group::InitLinks() {
  Con_Group::InitLinks();
  taBase::Own(wt_range, this);
}

void BioBelCon_Group::Copy_(const BioBelCon_Group& cp) {
  wt_gain = cp.wt_gain;
  wt_range = cp.wt_range;
}
...
void BioBelConSpec::Initialize() {
  ...
  wt_scale = 1.0f;
  gain_wt_dev_rng.min = .2f;
  gain_wt_dev_rng.max = .48f;
}

void BioBelConSpec::InitLinks() {
  ConSpec::InitLinks();
  taBase::Own(gain_wt_dev_rng, this);
}
...
void VChanSpec::Copy_(const VChanSpec& cp) {
  on = cp.on;
  inc = cp.inc;
  dec = cp.dec;
  gain = cp.gain;
...
float BioBelUnitSpec::Compute_IThresh(BioBelUnit* u) {
  ...
  // not including the ga and gh terms -- and not using real threshold
