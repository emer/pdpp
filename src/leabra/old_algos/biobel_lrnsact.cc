...
void SActInhibVals::Copy_(const SActInhibVals& cp) {
  gain = cp.gain;
  err = cp.err;
  sq_err = cp.sq_err;
  d_gain = cp.d_gain;
...
void BioBelLayerSpec::InitInhib(BioBelLayer* lay) {
  lay->i_sact.Initialize();
  lay->i_sact.gain = i_sact.gain;

  if(lay->units.gp.size > 0) {
    int g;
    for(g=0; g<lay->units.gp.size; g++) {
      BioBelUnit_Group* rugp = (BioBelUnit_Group*)lay->units.gp.FastEl(g);
      rugp->i_sact.Initialize();
      rugp->i_sact.gain = i_sact.gain;
    }
  }
}
...
void BioBelLayerSpec::Compute_Inhib_impl(BioBelLayer* lay, Unit_Group* ug, BioBelInhib* thr) {
  if((compute_i == KWTA_INHIB) || (compute_i == MAX_KWTA_SACT) || 
     (i_sact.lrn_frm == SActInhibSpec::LRN_KWTA))
    Compute_Inhib_kWTA(lay, ug, thr);
  // this is easy, so always compute it
  Compute_Inhib_sAct(lay, ug, thr);

  if(compute_i == KWTA_INHIB)
    thr->i_val.g_i = thr->i_val.kwta;
  else if(compute_i == SACT_INHIB)
    thr->i_val.g_i = thr->i_val.sact;
  else
    thr->i_val.g_i = MAX(thr->i_val.kwta, thr->i_val.sact);
...
void BioBelLayerSpec::Compute_Inhib_kWTA(BioBelLayer*, Unit_Group* ug, BioBelInhib* thr) {
  ...
  if(nw_gi >= thr->i_val.kwta)
    thr->i_val.kwta = nw_gi;
  else				// thresh goes down slowly
    thr->i_val.kwta += i_kwta_dtd * (nw_gi - thr->i_val.kwta);
...
void BioBelLayerSpec::Compute_Inhib_sAct(BioBelLayer* lay, Unit_Group*, BioBelInhib* thr) {
  float new_sact = thr->i_sact.gain * lay->sact.avg;
  thr->i_val.sact += i_sact.dt * (new_sact - thr->i_val.sact);
  if(i_sact.lrn_frm == SActInhibSpec::LRN_PCT)
    thr->i_sact.err = thr->acts.avg - thr->kwta.pct; // adj in negative dir of err
  else
    thr->i_sact.err = thr->i_val.kwta - thr->i_val.sact;
  thr->i_sact.sq_err = thr->i_sact.err * thr->i_sact.err;
}

void BioBelLayerSpec::Learn_Inhib_sAct(BioBelLayer* lay, Unit_Group*, BioBelInhib* thr) {
  thr->i_sact.d_gain = thr->i_sact.err * lay->sact.avg;
  thr->i_sact.gain += i_sact.lrate * thr->i_sact.d_gain;
  thr->i_sact.gain = MAX(thr->i_sact.gain, 0.0f);
}
...
void BioBelLayerSpec::PostSettle(BioBelLayer* lay, int phase) {
  if(lay->units.gp.size > 0) {
    ...
    for(g=0; g<lay->units.gp.size; g++) {
      ...
      Learn_Inhib_sAct(lay, rugp, (BioBelInhib*)rugp);
    ...
    Learn_Inhib_sAct(lay, &(lay->units), (BioBelInhib*)lay);
