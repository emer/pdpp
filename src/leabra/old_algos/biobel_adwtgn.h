...
public:
  float		wt_gain;	// gain value for the weights in this group
  MinMax	wt_range;	// range of the weight values
...
public:
  ...
  MinMax	gain_wt_dev_rng;
  // range of the max weight deviations from .5 beyond which wt_gain is adapted

  static float	WeightSigFun(float w, float gain) { // function for implementing weight sigmodi
    float wrat;
  ...
  static float	WtGainFmCurTrg(float cur, float trg) {
    return logf((1.0f - trg)/trg) / logf((1.0f - cur)/cur);
  }

  // compute weight gain from current and target weight values
  void		C_Compute_WtFmLin(BioBelCon_Group* cg, BioBelCon* cn)
  { cn->wt = WeightSigFun(cn->wt_lin, cg->wt_gain); }
  virtual void 	Compute_WtFmLin(BioBelCon_Group* cg, BioBelUnit*)
  ...
  { cn->wt_lin = WeightSigFun(cn->wt, 1.0f / cg->wt_gain); }
  virtual void 	Compute_LinFmWt(BioBelCon_Group* cg, BioBelUnit*)
  { CON_GROUP_LOOP(cg, C_Compute_LinFmWt(cg, (BioBelCon*)cg->Cn(i))); }
  // compute linear weight value from actual weight value (do after loading)
  ...
  void		C_UpdtWtMinMax(BioBelCon_Group* cg, BioBelCon* cn)
  { cg->wt_range.UpdateRange(cn->wt); }
  virtual void	UpdtWtMinMax(BioBelCon_Group* cg, BioBelUnit*) {
    cg->wt_range.min = 1.0f; cg->wt_range.max = 0.0f;
    CON_GROUP_LOOP(cg, C_UpdtWtMinMax(cg, (BioBelCon*)cg->Cn(i)));
  }
  // update min, max of weights in this layer

  virtual void	AdaptWtGain(BioBelCon_Group* cg, BioBelUnit* ru) {
    float min_dev = .5f - cg->wt_range.min;  float max_dev = cg->wt_range.max - .5f;
    float dev = MAX(max_dev, min_dev);
    float dev_lin = WeightSigFun(dev + .5f, 1.0f / cg->wt_gain); // deviation in linear terms
    if(dev > gain_wt_dev_rng.max)
      cg->wt_gain = WtGainFmCurTrg(dev_lin, gain_wt_dev_rng.max + .5f);
    else if(dev < gain_wt_dev_rng.min)
      cg->wt_gain = WtGainFmCurTrg(dev_lin, gain_wt_dev_rng.min + .5f);
  }
  // adapt the weight gain parameter based on gain_wt_dev_range 
  ...
  void 		InitWtState(Con_Group* cg, Unit* ru) {
    ...
    InitWtDelta(cg,ru);
    bcg->wt_gain = 1.0f;
    Compute_LinFmWt(bcg, (BioBelUnit*)ru);
    ApplySymmetry(cg,ru);
    UpdtWtMinMax(bcg, (BioBelUnit*)ru);
  }
...
public:
  ...
  void	Initialize() 	{ send = .1f; learn = 0.0f; }
  void	Destroy()	{ };
...
public:
  ...
  VChanSpec	acc_f;		// fast accomodation v-gated chan (delayed rectifier)
