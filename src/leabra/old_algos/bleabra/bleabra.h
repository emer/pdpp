...
public:
  ...
  inline virtual float Compute_SAvgCor(LeabraCon_Group* cg, float& dec_cor);
  ...
  inline float	C_Compute_Hebb(LeabraCon* cn, float ru_act, float su_act,
			       float inc_cor, float dec_cor);
  // compute Hebbian associative learning

  inline float 	C_Compute_Sig_Err(LeabraCon*, float ru_act_p, float ru_act_m,
				  float su_act_p, float su_act_m);
  // compute generec error term, sigmoid case

  inline float 	C_Compute_GausSig_Err(LeabraCon* cn, float ru_act_p, float ru_act_m,
				      float su_act_p, float su_act_m,
				      float ru_act_dif, float su_act_dif);
  // compute generec error term, GausSig act and wt lim case
  ...
  inline virtual void 	Compute_Sig_dWt(LeabraCon_Group* cg, LeabraUnit* ru);
  // compute weight change, sigmoidal units, rebel maxin
  inline virtual void 	Compute_GausSig_dWt(LeabraCon_Group* cg, LeabraUnit* ru);
  // compute weight change, gaussig, rebel maxin
...
public:
  ...
  void		Compute_Sig_dWt(LeabraUnit* ru)
  { ((LeabraConSpec*)spec.spec)->Compute_Sig_dWt(this, ru); }
  void		Compute_GausSig_dWt(LeabraUnit* ru)
  { ((LeabraConSpec*)spec.spec)->Compute_GausSig_dWt(this, ru); }
...
inline float LeabraConSpec::C_Compute_Hebb(LeabraCon* cn, float ru_act, float su_act,
					   float inc_cor, float dec_cor)
{
  return ru_act * (inc_cor * su_act * (1.0f - cn->wt) -
		   dec_cor * (1.0f - su_act) * cn->wt);
}

// generec error term with sigmoid activation function
inline float LeabraConSpec::C_Compute_Sig_Err
(LeabraCon*, float ru_act_p, float ru_act_m, float su_act_p, float su_act_m) {
  return (ru_act_p * su_act_p) - (ru_act_m * su_act_m);
}


// this includes x * w limitation as constants, not ignoring them altogether
inline float LeabraConSpec::C_Compute_GausSig_Err
(LeabraCon* cn, float ru_act_p, float ru_act_m, float su_act_p, float su_act_m,
 float ru_act_dif, float su_act_dif)
{
  float err = (ru_act_p * su_act_p - ru_act_m * su_act_m) + k_gaus * cn->wt * 
    (ru_act_dif * (su_act_p * (su_act_p - cn->wt) + su_act_m * (su_act_m - cn->wt)) +
     su_act_dif * (ru_act_p * (ru_act_p - cn->wt) + ru_act_m * (ru_act_m - cn->wt)));
...
inline void LeabraConSpec::Compute_Sig_dWt(LeabraCon_Group* cg, LeabraUnit* ru) {
  float dec_cor;
  float inc_cor = Compute_SAvgCor(cg, dec_cor);
  for(int i=0; i<cg->size; i++) {
    LeabraUnit* su = (LeabraUnit*)cg->Un(i);
    LeabraCon* cn = (LeabraCon*)cg->Cn(i);
    C_Compute_dWt(cn,
		  C_Compute_Hebb(cn, ru->act_p, su->act_p, inc_cor, dec_cor),
		  C_Compute_Sig_Err(cn, ru->act_p, ru->act_m, su->act_p, su->act_m));  
  }
}

inline void LeabraConSpec::Compute_GausSig_dWt(LeabraCon_Group* cg, LeabraUnit* ru) {
  float dec_cor;
  float inc_cor = Compute_SAvgCor((LeabraCon_Group*)cg, dec_cor);
  for(int i=0; i<cg->size; i++) {
    LeabraUnit* su = (LeabraUnit*)cg->Un(i);
    LeabraCon* cn = (LeabraCon*)cg->Cn(i);
    C_Compute_dWt(cn,
		  C_Compute_Hebb(cn, ru->act_p, su->act_p, inc_cor, dec_cor),
		  C_Compute_GausSig_Err(cn, ru->act_p, ru->act_m, su->act_p, su->act_m,
					ru->act_dif, su->act_dif));  
  }
}
...
public:
  ...
  inline void 	Compute_Sig_dWt(LeabraCon_Group* cg, LeabraUnit* ru);
  inline void 	Compute_GausSig_dWt(LeabraCon_Group* cg, LeabraUnit* ru);
...
inline void LeabraHistConSpec::Compute_Sig_dWt(LeabraCon_Group* cg, LeabraUnit* ru) {
  float dec_cor;
  float inc_cor = Compute_SAvgCor(cg, dec_cor);
  ...
  for(int i=0; i<cg->size; i++) {
    ...
    if(su->hist_cnt >= 1) {	// only do history computation if history is there
      C_Compute_dWt(cn, 
		    C_Compute_Hebb(cn, ru->act_p, su->act_p_h, inc_cor, dec_cor),
		    C_Compute_Sig_Err(cn, ru->act_p, ru->act_m, su->act_p_h, su->act_m_h));
    }
  }
  LeabraConSpec::Compute_Sig_dWt(cg, ru);
}

inline void LeabraHistConSpec::Compute_GausSig_dWt(LeabraCon_Group* cg, LeabraUnit* ru) {
  float dec_cor;
  float inc_cor = Compute_SAvgCor(cg, dec_cor);
  // this pass is for the compute sender_(t-1), recv_(t) weight change
  for(int i=0; i<cg->size; i++) {
    ...
    if(su->hist_cnt >= 1) {	// only do history computation if history is there
      C_Compute_dWt(cn, 
		    C_Compute_Hebb(cn, ru->act_p, su->act_p_h, inc_cor, dec_cor),
		    C_Compute_GausSig_Err(cn, ru->act_p, ru->act_m, su->act_p_h, su->act_m_h,
					  ru->act_dif, su->act_dif));
    }
  }
  LeabraConSpec::Compute_GausSig_dWt(cg, ru);
}
