...
public:
  ...
  float 	k_gaus;		// multilplier for the Gaussian aspect of GausSig
  ...
  inline float 	C_Compute_GausSig_Net(BioBelCon* cn, BioBelUnit*, BioBelUnit* su);
  inline virtual float	Compute_GausSig_Net(BioBelCon_Group* cg, BioBelUnit* ru);
  // GausSig with act and weight-limited difference term, receiver based!

  inline void 	C_Send_GausSig_Net(BioBelCon* cn, BioBelUnit*, BioBelUnit* su);
  inline virtual void	Send_GausSig_Net(BioBelCon_Group* cg, BioBelUnit* su);
  // GausSig with act and weight-limited difference term, sender based
...
public:
  float	Compute_GausSig_Net(BioBelUnit* u)
  { return ((BioBelConSpec*)spec.spec)->Compute_GausSig_Net(this, u); }

  void	Send_GausSig_Net(BioBelUnit* u)
  { ((BioBelConSpec*)spec.spec)->Send_GausSig_Net(this, u); }
...
public:
  ...
  float		thresh;	// threshold for starting to accumulate basis variable
  ...
  void	UpdateBasis(float& basis, float v_m) {
...
public:
  enum NetInFun {		// type of net-input computation to use
    DOT_PRODUCT,		// standard dot product net input
    GAUSSIG,			// Gaussian-Sigmoid hybrid, weight & act limited diff
    GAUSSIG_SEND		// sender-based gaussian-sigmoid hybrid
  };
  ...
  NetInFun	netin_fun;	// type of net input computation to use
  NetInFun	learn_fun;	// type of learning rule to use (appropriate for net in type)
  ...
  float		acc_s_gain;	// relative gain of the acc_s term compared to acc_f
  ...
  float		send_thresh;	// sending activity threshold (don't send act below this)
...
public:
  ...
  float		hyst;		// #NO_VIEW hysteresis basis variable
  float		acc_f;		// #NO_VIEW fast accomodation basis variable
  float		acc_s;		// #NO_VIEW slow accomodation basis variable
  float		g_l;		// #NO_SAVE #NO_VIEW net leak conductance
  float		g_i;		// #NO_SAVE net inhibitory conductance
  float		g_h;		// #NO_SAVE hysteresis conductance
...
// GausSig with weight & act limited difference term, receiver based
float BioBelConSpec::C_Compute_GausSig_Net(BioBelCon* cn, BioBelUnit*, BioBelUnit* su) {
  float a_dif = su->act - cn->wt;
  return (su->act * cn->wt) * (1.0f - (k_gaus * a_dif * a_dif));
}
float BioBelConSpec::Compute_GausSig_Net(BioBelCon_Group* cg, BioBelUnit* ru) {
  float rval=0.0f;
  CON_GROUP_LOOP
    (cg, rval += C_Compute_GausSig_Net((BioBelCon*)cg->Cn(i), ru,(BioBelUnit*)cg->Un(i)));
  return wt_gain * rval;
}

// GausSig with act and weight-limited difference term, sender based
void BioBelConSpec::C_Send_GausSig_Net(BioBelCon* cn, BioBelUnit* ru, BioBelUnit* su) {
  float a_dif = su->act - cn->wt;
  ru->net += wt_gain * ((su->act * cn->wt) * (1.0f - (k_gaus * a_dif * a_dif)));
}
void BioBelConSpec::Send_GausSig_Net(BioBelCon_Group* cg, BioBelUnit* su) {
  CON_GROUP_LOOP(cg, C_Send_GausSig_Net((BioBelCon*)cg->Cn(i), (BioBelUnit*)cg->Un(i), su));
}
