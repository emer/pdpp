...
void LeabraConSpec::Initialize() {
  ...
  k_hebb = .1f;
  k_err = 1.0f - k_hebb;
  savg_src = SLAYER_TRG_PCT;
  fixed_savg = .25f;
  fx_inc_cor = sqrtf((1.0f - fixed_savg) / fixed_savg);
  fx_dec_cor = 1.0f / fx_inc_cor;
}
...
void LeabraUnitSpec::Compute_dWt_impl(LeabraUnit* u) {
  ...
  int g;
  if(learn_fun == DOT_PRODUCT) {
    FOR_ITR_GP(LeabraCon_Group, recv_gp, u->recv., g) {
      if(!recv_gp->prjn->from->lesion)
	recv_gp->Compute_Sig_dWt(u);
    }
  }
  else {			// GAUSSIG
    FOR_ITR_GP(LeabraCon_Group, recv_gp, u->recv., g) {
      if(!recv_gp->prjn->from->lesion)
	recv_gp->Compute_GausSig_dWt(u);
    }
  }
}
