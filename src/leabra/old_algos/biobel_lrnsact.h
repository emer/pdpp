...
public:
  ...
  float		g_h;		// #NO_SAVE #NO_VIEW hysteresis conductance
  float		g_a;		// #NO_SAVE net accomodation conductance
  float		g_l;		// #NO_SAVE #NO_VIEW net leak conductance
  float		g_i;		// #NO_SAVE #NO_VIEW net inhibitory conductance
...
class SActInhibSpec : public taBase {
  // #INLINE ##NO_TOKENS #NO_UPDATE_AFTER holds parameters for sact inhibition
public:
  enum LearnFrom {
    LRN_PCT,			// learn based on target activity percentage
    LRN_KWTA			// learn based on value computed by kwta
  };

  LearnFrom	lrn_frm;	// what to learn inhib rate from
  float		dt;		// rate constant for integrating inhibition
  float		gain;		// initial value for gain function of sact
  float		lrate;		// rate at which gain is learned

  void	Initialize();
  void 	Destroy()	{ };
  SIMPLE_COPY(SActInhibSpec);
  COPY_FUNS(SActInhibSpec, taBase);
  TA_BASEFUNS(SActInhibSpec);
};

class SActInhibVals : public taBase {
  // #INLINE ##NO_TOKENS #NO_UPDATE_AFTER holds values for sact inhibition
public:
  float		gain;		// gain of sending act based inihbition
  float		err;		// error in sact compared to kwta inhib
  float		sq_err;		// squared error
  float		d_gain;		// change in inhibitory gain

  void	Initialize();
  void 	Destroy()	{ };
  void	Copy_(const SActInhibVals& cp);
  COPY_FUNS(SActInhibVals, taBase);
  TA_BASEFUNS(SActInhibVals);
};
...
public:
  enum Compute_I {		// how to compute the inhibition
    ...
    MAX_KWTA_SACT		// set as max of both kwta and sact
  };
  ...
  float		i_kwta_dtd;	// time constant for decreases in threshold
  SActInhibSpec i_sact;		// parameters for sending act based inhbition
  ...
  virtual void	Learn_Inhib_sAct(BioBelLayer* lay, Unit_Group* ug, BioBelInhib* thr);
  // learn the sAct inhibition based on kwta inhibition
...
public:
  ...
  SActInhibVals	i_sact;		// inhibition due to sending activation
