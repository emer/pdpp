...
void VChanSwitch::Copy_(const VChanSwitch& cp) {
  hyst = cp.hyst;
  rec = cp.rec;
  acc_f = cp.acc_f;
  acc_s = cp.acc_s;
...
void BioBelUnitSpec::InitWtState(Unit* u) {
  ...
  bu->vcb.rec = .5f * rec.d_thr;
  bu->vcb.acc_s = .5f * acc_s.d_thr;
  bu->vcs.rec = false;
  bu->vcs.acc_s = false;
}
				  ...
				  int cycle)
{
  ...
  float g_a = acc_f.Compute_G(u->vcb.acc_f, u->vcs.acc_f) +
    acc_s.Compute_G(u->vcb.acc_s, u->vcs.acc_s);
  float	g_h = hyst.Compute_G(u->vcb.hyst, u->vcs.hyst) +
    rec.Compute_G(u->vcb.rec, u->vcs.rec);
  ...
  hyst.UpdateBasis(u->vcb.hyst, u->vcs.hyst, u->act);
  acc_f.UpdateBasis(u->vcb.acc_f, u->vcs.acc_f, u->act);
}
...
// 	   + u->g_a * (e_rev.a - act.thresh) + u->g_h * (e_rev.h - act.thresh)) /
// 	  (act.thresh - e_rev.i));
...
void BioBelUnitSpec::DecayState(BioBelUnit* u, BioBelLayer*, float decay) {
  ...
  //  u->vcb.rec -= decay * u->vcb.rec;
  u->vcb.acc_f -= decay * u->vcb.acc_f;
  //  u->vcb.acc_s -= decay * u->vcb.acc_s;
...
void BioBelUnitSpec::PostSettle(BioBelUnit* u, BioBelLayer*, BioBelInhib*, int) {
  // slow time-scale updated after settling (done only in plus phase for leabra)
  rec.UpdateBasis(u->vcb.rec, u->vcs.rec, u->act);
  acc_s.UpdateBasis(u->vcb.acc_s, u->vcs.acc_s, u->act);
}
