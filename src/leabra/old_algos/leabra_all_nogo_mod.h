...
public:
  ...
  inline float C_Compute_Hebb(LeabraCon* cn, LeabraCon_Group* cg, DaModUnit* ru, DaModUnit* su) {
    // wt is negative in linear form, so using opposite sign of usual here
    float rval;
    if((learn_rule == MOTOR_DELTA) || (learn_rule == MOTOR_CHL))
...
public:
  ...
  float		rgo_da;		// #DEF_10 strength of DA for activating Go (gc.h) and inhibiting NoGo (gc.a) when max_mnt is up or a random Go is active (applied during learning, after Go/NoGo already computed)
...
public:
  ...
  float		all_nogo_mod;	// #DEF_0.5 if all snr units fire nogo, then modulate everyone's by this amount * -DA
...
public:
  ...
  // compute gating signal for MOTOR Matrix_out
...
public:
  ...
  float		tie_thr;	// #DEF_0.2 how high does the average netinput need to be to detect a tie-deadlock among a set of snrthal units all sending equally strong GO signals, such that the net activation is < go_thr?
  bool		tie_max;	// #DEF_false choose the max active units to win in the case of a snrthal tie deadlock 
...
public:
  ...
  virtual void	BreakTies(LeabraLayer* lay, LeabraTrial* trl);
  // break ties in Go firing due to too many units trying to fire (happens in PostSettle, just before gating)
