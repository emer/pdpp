...
void LeabraUnitSpec::Send_Net(LeabraUnit* u, LeabraLayer*, LeabraLayer* tolay) {
  ...
  if(u->act > opt_thresh.send) {
    ...
    FOR_ITR_GP(LeabraCon_Group, send_gp, u->send., g) {
      ...
      if(tol->lesion || tol->hard_clamped || (tol != tolay))	continue;
      send_gp->Send_Net(u);
    }
  }
}
...
void LeabraLayerSpec::Send_Net(LeabraLayer* lay, LeabraLayer* tolay, int, int) {
...
void LeabraLayerSpec::Compute_Inhib(LeabraLayer* lay, int, int phase) {
  if(lay->hard_clamped)	return;	// say no more..

  if(inhib_group != UNIT_GROUPS) {
    Compute_Inhib_impl(lay, &(lay->units), (LeabraInhib*)lay, phase);
  }
  if(lay->units.gp.size > 0) {
    int g;
	...
	for(g=0; g<lay->units.gp.size; g++) {
	  LeabraUnit_Group* rugp = (LeabraUnit_Group*)lay->units.gp.FastEl(g);
	  Compute_Inhib_impl(lay, rugp, (LeabraInhib*)rugp, phase);
...
void LeabraLayerSpec::Compute_Inhib_impl(LeabraLayer* lay, Unit_Group* ug, LeabraInhib* thr, int phase) {
  // for the time being, always compute kwta just to have it around..
  if(compute_i == KWTA_AVG_INHIB)
    Compute_Inhib_kWTA_Avg(lay, ug, thr, phase);
  else
    Compute_Inhib_kWTA(lay, ug, thr, phase);
...
void LeabraLayerSpec::Compute_Inhib_kWTA(LeabraLayer*, Unit_Group* ug, LeabraInhib* thr, int phase) {
...
void LeabraLayerSpec::Compute_Inhib_kWTA_Avg(LeabraLayer*, Unit_Group* ug, LeabraInhib* thr, int phase) {
  if(ug->leaves <= 1) {	// this is undefined
    thr->Inhib_SetVals(i_kwta_pt);
    return;
  }
...
//  static void par_compute_clamp_netavg(PThread* pt) {
//    LeabraCycle* cyc = (LeabraCycle*)pt->proc;
//    int i;
//    for(i=0;i<pt->layers.size;i++) {
//      LeabraLayer* lay = (LeabraLayer*)pt->GetLay(i);
//      lay->Compute_Clamp_NetAvg(cyc->leabra_settle->cycle.val,
//  			      (int)cyc->leabra_settle->leabra_trial->phase);
//    }
//  }

// keeping this non-parallel cuz its very little computation, not worth overhead
...
static void par_compute_inhib(PThread* pt) {
  LeabraCycle* cyc = (LeabraCycle*)pt->proc;
  int i;
  for(i=0;i<pt->layers.size;i++) {
    LeabraLayer* lay = (LeabraLayer*)pt->GetLay(i);
    lay->Compute_Inhib(cyc->leabra_settle->cycle.val,
		     (int)cyc->leabra_settle->leabra_trial->phase);
  }
}
...
void LeabraCycle::Compute_Inhib() {
  if((leabra_settle == NULL) || (leabra_settle->leabra_trial == NULL))
    return;
  network->RunPThreadsUnt(par_compute_inhib, this);
}  

static void par_compute_act(PThread* pt) {
  LeabraCycle* cyc = (LeabraCycle*)pt->proc;
  int i;
  for(i=0;i<pt->layers.size;i++) {
    LeabraLayer* lay = (LeabraLayer*)pt->GetLay(i);
    lay->Compute_Act(cyc->leabra_settle->cycle.val,
		     (int)cyc->leabra_settle->leabra_trial->phase);
  }
}
...
void LeabraCycle::Compute_Act() {
  if((leabra_settle == NULL) || (leabra_settle->leabra_trial == NULL))
    return;
  network->RunPThreadsUnt(par_compute_act, this);
}
...
static void par_compute_wtfmlin(PThread* pt) {
  int i;
  for(i=0;i<pt->layers.size;i++) {
    LeabraLayer* lay = (LeabraLayer*)pt->GetLay(i);
    lay->Compute_WtFmLin();
...
void LeabraSettle::Compute_WtFmLin() {
  network->RunPThreadsCon(par_compute_wtfmlin, this);
}
...
static void par_compute_dwt_post(PThread* pt) {
  int i;
  for(i=0;i<pt->layers.size;i++) {
    LeabraLayer* lay = (LeabraLayer*)pt->GetLay(i);
...
void LeabraTrial::Compute_dWt_post() {
  network->RunPThreadsCon(par_compute_dwt_post, this);
}
...
void LeabraTrial::Final() {
  ...
  if((epoch_proc != NULL) && (epoch_proc->wt_update != EpochProcess::TEST)
    ...
    UpdateWeights();		// always update weights after each trial of learning to have
				// weights of correct sign for viewing (checks in place for not
				// redoing at epoch level, in updateweights)
  }
}
...
void LeabraContextLayerSpec::Compute_HardClamp(LeabraLayer* lay, LeabraTrial* trl) {
  if(trl->trial_init != LeabraTrial::DECAY_STATE) {
    taMisc::Error("LeabraTrial: LeabraContextLayerSpec requires trial_init = DECAY_STATE, I just set it for you");
    trl->trial_init = LeabraTrial::DECAY_STATE;
  }
  lay->hard_clamped = true;	// cache this flag
  lay->SetExtFlag(Unit::EXT);
...
void LeabraGatedCtxLayerSpec::Compute_HardClamp(LeabraLayer* lay, LeabraTrial* trl) {
  ...
  if(trl->phase_order != LeabraTrial::MINUS_PLUS_PLUS) {
    taMisc::Error("LeabraTrial: LeabraContextLayerSpec requires phase_oder = MINUX_PLUS_PLUS, I just set it for you");
...
void LeabraGatedCtxLayerSpec::FindACLayer(LeabraLayer* lay) {
...
void LeabraActRegUnitSpec::Initialize() {
}

void LeabraActRegUnitSpec::InitLinks() {
  LeabraUnitSpec::InitLinks();
  taBase::Own(act_reg, this);
}

void LeabraActRegUnitSpec::Compute_dWt(LeabraUnit* u, LeabraLayer* lay) {
  LeabraUnitSpec::Compute_dWt(u, lay);
  if(u->ext_flag & (Unit::EXT | Unit::TARG))
    return;			// don't adapt thresholds for clamped layers
  float bdt = act_reg.bias_dt;
  if(act_reg.dt_var > 0.0f)
    bdt += Random::Gauss(act_reg.dt_var);
  if(u->act_avg < act_reg.min)
    u->bias->wt += bdt;
  else if(u->act_avg > act_reg.max)
    u->bias->wt -= bdt;
}  
