...
class BioBelCon : public Connection {
  // BioBel connection
public:
  float		wt_lin;		// linear (internal) weight value (on which learning occurs)
...
public:
  float		wt_off;		// dynamically updated  weight offset parameter
  int		n_req;		// #NO_SAVE number of cons required to change wt_off values
  int		n_ov_min;	// #NO_SAVE number of cons over min weight value 
  int		n_ov_max;	// #NO_SAVE number of cons over max weight value
...
class WtOffSpec : public taBase {
  // ##INLINE ##NO_TOKENS #NO_UPDATE_AFTER parameters for updating wt_off parameter
public:
  float		min;		// minimum weight value (if not pct above min, wt_off reduced)
  float		max;		// maximum weight value (if pct above max, wt_off increased)
  float		pct;		// percentage of connections necessary for inc/dec
  float		d_o;		// delta-offset: amount to change wt_off if out of range (.001 typical)
  float		off;		// initial wt offset value

  void		UpdateWtOff(float& wt_off, int n_req, int n_ov_min, int n_ov_max) {
    if(n_ov_max >= n_req) 
      wt_off += d_o;
    else if ((n_ov_min + n_ov_max) < n_req)
      wt_off -= d_o;
  }

  void	Initialize();
  void	Destroy()	{ };
  SIMPLE_COPY(WtOffSpec);
  COPY_FUNS(WtOffSpec, taBase);
  TA_BASEFUNS(WtOffSpec);
};
...
public:
  ...
  float		wt_gain;	// gain of the weight sigmoidal function
  WtOffSpec	wt_off;		// controls offset that moves sigmoidal function offset (>1 moves up)
  MinMax	wt_off_rng;	// range to keep wt_off parameter within
  FunLookup	wt_sig_fun;	// #HIDDEN #NO_SAVE #NO_INHERIT computes wt sigmoidal fun

  // function for implementing weight sigmodid
  static float	WeightSigFun(float w, float gain, float off) {
    float wrat;
  ...
  static float	WeightSigFunInv(float w, float gain, float off) {
    float wrat;
    if(w == 0.0f) wrat = MAXFLOAT;
    else if(w == 1.0f) wrat = 0.0f;
    else wrat = (1.0f - w) / w;
  ...
  void		C_Compute_WtFmLin(BioBelCon_Group* cg, BioBelCon* cn) {
    if(wt_off.d_o == 0.0f)	// not adapting wt_off, use faster pre-compiled fun
      cn->wt = wt_sig_fun.Eval(cn->wt_lin);
    else
      cn->wt = WeightSigFun(cn->wt_lin, wt_gain, cg->wt_off);
  }
  // compute actual weight value from linear weight value

  void		C_Compute_LinFmWt(BioBelCon_Group* cg, BioBelCon* cn)
  { cn->wt_lin = WeightSigFunInv(cn->wt, wt_gain, cg->wt_off); }
  // compute linear weight value from actual weight value

  void 		C_InitWtState(Connection* cn, Unit* ru, Unit* su) {
    ConSpec::C_InitWtState(cn, ru, su); }
  inline virtual void	C_InitWtState(BioBelCon* cn, BioBelCon_Group* cg,
				      BioBelUnit* ru, BioBelUnit* su);
  ...
  virtual void	Compute_WtOff(BioBelCon_Group* cg, BioBelUnit*) {
    cg->n_ov_min = 0;	cg->n_ov_max = 0;
    for(int i=0; i<cg->size; i++) {
      if(cg->Cn(i)->wt > wt_off.max) cg->n_ov_max++;
      else if(cg->Cn(i)->wt > wt_off.min) cg->n_ov_min++;
    }
    cg->n_req = (int)(wt_off.pct * (float)cg->size);
    wt_off.UpdateWtOff(cg->wt_off, cg->n_req, cg->n_ov_min, cg->n_ov_max);
    cg->wt_off = wt_off_rng.Clip(cg->wt_off);
  }
...
public:
  ...
  float		a_thr;		// activation threshold of the channel (0 = graded)
  float		d_thr;		// deactivation threshold of the channel (0 = graded)

  float	Compute_G(float basis, bool on_off) { // get the conductance from this channel
    float rval = 0.0f;
    if(on) {
      if(a_thr == 0.0f)
	rval = basis;	// if not thresholded
      else
	rval = on_off ? 1.0f : 0.0f; // either on or off
    }
    return rval;
  }
...
public:
  ...
  bool		hyst_on;	// binary thresholded mode state variable, hyst
  bool		acc_on;		// binary thresholded mode state variable, acc
...
public:
  ...
  float		da;		// #NO_SAVE delta-activation (actually memb potential)
...
public:
  int		NetCompare(BioBelUnit* a, BioBelUnit* b) const
  { int rval=-1; if(a->net < b->net) rval=1;
    else if(a->net == b->net) rval=0; return rval; }
  // compare two items for purposes of sorting: descending order by net

  void	SortByNet();		// sort list by net input
...
public:
  ...
  float		gap_q;		// threshold for gap size above which k moves to this gap between units
...
public:
  ...
  int		act_k;		// actual number of active units, as determined by gap
  float		pct;		// actual percent activity in group
  float		pct_c;		// #HIDDEN complement of (1.0 - ) actual percent activity in group
  float		Ok_gap;		// gap value between highest and kth highest units (0-k gap)
  float		max_gap;	// maximum gap size between adjacent units (as prop of 0-k gap)
...
void BioBelConSpec::C_InitWtState(BioBelCon* cn, BioBelCon_Group* cg, BioBelUnit* ru, BioBelUnit* su) {
  ConSpec::C_InitWtState(cn, ru, su);
  C_Compute_LinFmWt(cg, cn);
}

void BioBelConSpec::InitWtState(Con_Group* cg, Unit* ru) {
  BioBelCon_Group* bcg = (BioBelCon_Group*)cg;
  bcg->wt_off = wt_off.off;	// initialize to starting value
  CON_GROUP_LOOP(bcg, C_InitWtState((BioBelCon*)cg->Cn(i), bcg,
				    (BioBelUnit*)ru, (BioBelUnit*)cg->Un(i)));
  InitWtDelta(cg,ru);
  ApplySymmetry(cg,ru);
}
