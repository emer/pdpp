...
public:
  ...
  enum SoftBound {
    SB_ERR,			// softbound the error
    SB_NONE			// no soft-bounding
  };

  enum HebbType {
    PSR_MAX_COR,		// prob of send given recv, correct for s_avg by larger wt max
    PSR_ID_COR			// psr, correct for s_avg by increase-decrease weighting factors
  };
  ...
  SoftBound	soft_bound;	// type of soft bounding to use
  HebbType	hebb_type;	// type of hebbian learning rule to use
  SAvgSource	savg_src;	// source of sending average act for correcting computation of psr
  float		fixed_savg;	// parameter for fixed sending average act
  float		k_savg_cor;	// how much to weight the savg correction factor

  void 		C_InitWtState(Connection* cn, Unit* ru, Unit* su) {
    ConSpec::C_InitWtState(cn, ru, su); }
  void 		C_InitWtState(BioBelCon* cn, BioBelCon_Group* cg, BioBelUnit* ru, BioBelUnit* su) {
    BioBelConSpec::C_InitWtState(cn, cg, ru, su); LeabraCon* lcn = (LeabraCon*)cn;
    lcn->pdw = 0.0f; }
...
public:
  ...
  float		inc_cor;	// #NO_SAVE savg increase correction factor for psr_id_cor
  float		dec_cor;	// #NO_SAVE savg decrease correction factor for psr_id_cor
...
inline void LeabraConSpec::Compute_SAvgCor(LeabraCon_Group* cg, LeabraUnit*) {
  ...
  cg->savg = .5f + k_savg_cor * (savg - .5f);
  cg->max_cor = .5f / cg->savg;
  cg->inc_cor = sqrtf((1.0f - cg->savg) / cg->savg);
  cg->dec_cor = 1.0f / cg->inc_cor;
}
...
{
  float hebb;
  if(hebb_type == PSR_MAX_COR)
    hebb = ru_act * (su_act * (cg->max_cor - cn->wt_lin) -
		     (1.0f - su_act) * cn->wt_lin);
  else
    hebb = ru_act * (cg->inc_cor * su_act * (1.0f - cn->wt) -
                     cg->dec_cor * (1.0f - su_act) * cn->wt);
  return hebb;
}
...
inline void LeabraConSpec::C_Compute_dWt(LeabraCon* cn, float heb, float err) {
  float dwt = k_err * err + k_hebb * heb;
  cn->dwt += dwt;
}
...
//  ApplyLimits(cg, ru); limits are automatically enforced anyway
  Compute_WtOff(lcg, (BioBelUnit*)ru);
}
...
// combine hebbian and error-driven, with hist_gain
inline void LeabraHistConSpec::C_Compute_dWt(LeabraCon* cn, float heb, float err) {
  float dwt = hist_gain * (k_err * err + k_hebb * heb);
  cn->dwt += dwt;
}
...
inline void LeabraHistConSpec::Compute_dWt(Con_Group* cg, Unit* ru) {
  ...
  for(int i=0; i<cg->size; i++) {
    ...
    if(su->hist_cnt >= 1) {	// only do history computation if history is there
      C_Compute_dWt(cn, 
		    C_Compute_Hebb(cn, lcg, lru->act_p, su->act_p_h),
