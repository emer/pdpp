...
void BioBelConSpec::Initialize() {
  ...
  wt_scale = 1.0f;
  regain_thr = .2f;
  wteff_fun.x_range.min = 0.0f;
  wteff_fun.x_range.max = 1.0f;
  wteff_fun.res = .001f;
  wteff_fun.x_range.UpdateAfterEdit();
}

void BioBelConSpec::InitLinks() {
  ConSpec::InitLinks();
  taBase::Own(wteff_fun, this);
  CreateWtEffFun();
}
...
void BioBelConSpec::CreateWtEffFun() {
  wteff_fun.AllocForRange();
  int i;
  for(i=0; i<wteff_fun.size; i++) {
    float w = wteff_fun.x_range.min + ((float)i * wteff_fun.res);
    wteff_fun.FastEl(i) = wt_scale * WeightGainFun(w, wt_gain);
  }
}
...
void AdaptThreshSpecs::Initialize() {
  dt = .005f;
  min = .05f;
  max = .35f;
  dthr = .001f;
				  ...
				  int cycle)
{
  ...
  float g_a = acc_f.Compute_G(u->vcb.acc_f);
  float	g_h = hyst.Compute_G(u->vcb.hyst);

  // total conductances
  u->gc.l = g_bar.l * (1.0f + bias_val);
  ...
  hyst.UpdateBasis(u->vcb.hyst, u->act);
  acc_f.UpdateBasis(u->vcb.acc_f, u->act);
}
...
void BioBelUnitSpec::AdaptThreshold(BioBelUnit* u, BioBelLayer*, BioBelInhib*, int) {
  if(u->act_avg < adapt_thr.min) {
    if(u->act < .5f)		// only decrease if unit actually off..
      u->thr -= adapt_thr.dthr;
  }
  else if(u->act_avg > adapt_thr.max) {
    if(u->act > .5f)		// only increase if unit actually on..
      u->thr += adapt_thr.dthr;
  }
  else {
    // decay the threshold back towards normal
    u->thr += adapt_thr.dthr * (act.thresh - u->thr);
  }
}

void BioBelUnitSpec::PostSettle(BioBelUnit* u, BioBelLayer* lay, BioBelInhib* thr,
				int phase)
{
  // this is replaced by plus-phase value in leabra
