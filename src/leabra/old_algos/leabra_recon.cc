...
FunLookup LeabraConSpec::log_fun;
...
void LeabraConSpec::Initialize() {
  ...
  k_hebb = .1f;
  k_rec = 0.0f;
  k_err = 1.0f - (k_hebb + k_rec);
  wt_inc_sc = sqrtf(2.0f);
  wt_dec_sc = 1.0f / wt_inc_sc;
  rec_deriv = false;
}
...
void LeabraConSpec::InitLogFun() {
  if(log_fun.size > 0)
    return;
  log_fun.x_range.min = 0.0f;
  log_fun.x_range.max = 1.0f;
  log_fun.res = .001f;
  log_fun.x_range.UpdateAfterEdit();
  log_fun.AllocForRange();
  int i;
  for(i=0; i < log_fun.size; i++) {
    float x = log_fun.x_range.min + ((float)i * log_fun.res);
    log_fun.FastEl(i) = logf(x);
  }
}

void LeabraConSpec::FindScPct(float pct) {
  float newsc = sqrtf(pct / .5f);
  wt_dec_sc = newsc;
  wt_inc_sc = 1.0f / newsc;
  UpdateAfterEdit();
}
...
void LeabraUnitSpec::InitWtState(Unit* u) {
  ...
  lu->act_p = lu->act_m = lu->act_dif = lu->recon = 0.0f;
}
...
void LeabraUnitSpec::Compute_dWt(Unit* u) {
  LeabraUnit* lu = (LeabraUnit*)u;
  if((hebb_type == HEBBIAN) && (lu->act_p <= learn_thresh) && (lu->act_m <= learn_thresh))
    return;
  Compute_dWt_impl(lu);
}
...
void LeabraUnitSpec::Compute_dWt_impl(LeabraUnit* u) {
  ...
  if(!((u->ext_flag & Unit::EXT) && !(u->ext_flag & Unit::TARG))) {
    if(bias_wts == BIAS_LEAK)
      ((LeabraConSpec*)bias_spec.spec)->B_Compute_dWt_Leak((LeabraCon*)u->bias, u);
    else
      ((LeabraConSpec*)bias_spec.spec)->B_Compute_dWt_Netin((LeabraCon*)u->bias, u);
  }
  
  if(hebb_type == HEBBIAN) {
    ...
    FOR_ITR_GP(LeabraCon_Group, send_gp, u->send., g) {
      ...
      send_gp->Compute_Recon(u);
    }
  }
}

void LeabraUnitSpec::Compute_dWt_post(LeabraUnit* u, LeabraLayer*) {
  if(hebb_type == HEBBIAN)
    return;

  // summarize the recon value for viewing..
  u->recon = 0.0f;
  float cnt = 0.0f;
  LeabraCon_Group* send_gp;
  int g;
...
void LeabraUnitSpec::UpdateWeights(Unit* u) {
  ...
  if(!((lu->ext_flag & Unit::EXT) && !(lu->ext_flag & Unit::TARG))) {
    if((lu->act_p > learn_thresh) || (lu->act_m > learn_thresh))
      ((LeabraConSpec*)bias_spec.spec)->B_UpdateWeights((LeabraCon*)lu->bias, lu);
    else
      ((LeabraCon*)lu->bias)->pdw = 0.0f; // for display purposes -- convenient
  }
  if(((lu->act_p > learn_thresh) || (lu->act_m > learn_thresh)))
...
void LeabraUnit::Initialize() {
  act_p = act_m = act_dif = recon = 0.0f;
...
void LeabraLayerSpec::PostSettle(BioBelLayer* lay, int phase) {
  ...
  // get the average..
  llay->acts_avg.avg = 0.5f * (llay->acts_p.avg + llay->acts_m.avg);
  llay->acts_avg.max = 0.5f * (llay->acts_p.max + llay->acts_m.max);
}
...
void LeabraPreSynConSpec::Initialize() {
  k_pre = .5f;
  k_pre_c = 1.0f - k_pre;
  pre_inc_sc = 1.0f;
  pre_dec_sc = 1.0f / pre_inc_sc;
}

void LeabraPreSynConSpec::UpdateAfterEdit() {
...
void LeabraHistUnitSpec::Compute_dWt(Unit* u) {
  ...
  if((lu->act_p <= learn_thresh) && (lu->act_m <= learn_thresh)) {
    if(lu->hist_cnt >= 1) {
      if((lu->act_p_h <= learn_thresh) && (lu->act_m_h <= learn_thresh))
	return;
    }
    else
      return;
...
void LeabraReScaleConSpec::Initialize() {
  min_obj_type = &TA_LeabraReScaleCon_Group;
  rescale_rate = 0.1f;
  rescale_max = .75f;
}

void LeabraReScaleCon_Group::Initialize() {
  max_wt = 0.0f;
  scale_factor = 0.0f;
}

void LeabraReScaleCon_Group::Copy_(const LeabraReScaleCon_Group& cp) {
  max_wt = cp.max_wt;
