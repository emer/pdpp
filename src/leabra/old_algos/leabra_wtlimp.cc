...
void LeabraConSpec::Initialize() {
  ...
  k_err = 1.0f;
  k_assoc = .1f;
  wt_lim_sc_i = sqrtf(2.0f);
  wt_lim_sc_d = 1.0f / wt_lim_sc_i;

  // todo: eliminate..
  wt_lim_p = 1.0f;
  pow_fun.x_range.min = 0.0f;
  pow_fun.x_range.max = 1.0f;
  pow_fun.res = .001f;
  pow_fun.x_range.UpdateAfterEdit();
}

void LeabraConSpec::InitLinks() {
  BioBelConSpec::InitLinks();
  taBase::Own(pow_fun, this);
}
...
void LeabraConSpec::CreatePowFun() {
  pow_fun.AllocForRange();
  int i;
  for(i=0; i < pow_fun.size; i++) {
    float x = pow_fun.x_range.min + ((float)i * pow_fun.res);
    pow_fun.FastEl(i) = powf(x, wt_lim_p);
  }
}
...
void LeabraLayerSpec::Compute_dWt_post(BioBelLayer* lay) {
  LeabraUnit* u;
  taLeafItr i;
  FOR_ITR_EL(LeabraUnit, u, lay->units., i)
...
void LeabraSettle::InitLinks() {
  ...
  if(!taMisc::is_loading && (loop_stats.size == 0)) {
    BioBelMaxDa* st = (BioBelMaxDa*)loop_stats.NewEl(1, &TA_BioBelMaxDa);
  }
}
...
void LeabraPreSynConSpec::Initialize() {
  ...
  k_pre_c = 1.0f - k_pre;
  pre_lim_sc_i = 1.0f;
  pre_lim_sc_d = 1.0f / pre_lim_sc_i;
}
...
void LeabraHistUnitSpec::Compute_dWt_post(LeabraUnit* u, BioBelLayer* lay) {
