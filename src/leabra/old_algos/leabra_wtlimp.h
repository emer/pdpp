...
public:
  ...
  float		k_err;		// strength of the error component
  float		k_assoc;	// strength of the associative component
  float		wt_lim_sc_i;	// postsynaptic scaling for weight increase
  float		wt_lim_sc_d;	// #HIDDEN 1.0 / sc_i = scaling for weight decrease
  float		wt_lim_p;	// power (exponent) of weight increase/decrease term
  FunLookup 	pow_fun;	// #HIDDEN #NO_SAVE #NO_INHERIT fast function lookup for pow
  ...
  virtual void		CreatePowFun();	// compute pow fun for lookup
...
public:
  ...
  virtual void	Compute_dWt_post(LeabraUnit*, BioBelLayer*)	{ };
  // after computing the weight changes (for sending weight layer competition, and hist)
...
public:
  ...
  void 		Compute_dWt_post(BioBelLayer* lay) 
...
public:
  ...
  virtual void	Compute_dWt_post(BioBelLayer* lay);
...
{
  return ru_act * (wt_lim_sc_i * su_act * (1.0f - cn->wt) -
		   wt_lim_sc_d * (1.0f - su_act) * cn->wt);
}
...
inline void LeabraConSpec::C_Compute_dWt(LeabraCon* cn, float mxi, float err) {
//   if(cond_assoc) {
//     float tmp = k_err * err;
//     if((tmp >= 0.0f) || (mxi < 0.0f)) // only if err positive or mxi neg..
//       tmp += k_assoc * mxi;
//     cn->dwt += tmp;
//   }
//   else {
    cn->dwt += k_err * err + k_assoc * mxi;
//  }
}
...
public:
  ...
  float		pre_lim_sc_i;	// presynaptic scaling for weight increase
  float		pre_lim_sc_d;	// #HIDDEN 1.0 / sc_i = scaling for weight decrease
...
{
  return k_pre_c * ru_act * (wt_lim_sc_i * su_act * (1.0f - cn->wt) -
				wt_lim_sc_d * (1.0f - su_act) * cn->wt) +
    k_pre * su_act * (pre_lim_sc_i * ru_act * (1.0f - cn->wt) -
			 pre_lim_sc_d * (1.0f - ru_act) * cn->wt);
}
...
public:
  ...
  void		Compute_dWt(Unit* u);
  void		Compute_dWt_post(LeabraUnit* u, BioBelLayer* lay);
...
inline void LeabraHistConSpec::C_Compute_dWt(LeabraCon* cn, float mxi, float err) {
  cn->dwt += hist_gain * (k_err * err + k_assoc * mxi);
}
