...
void LeabraUnitSpec::Compute_Conduct(LeabraUnit* u, LeabraLayer* lay, LeabraInhib*, LeabraTrial*) {
  // total conductances
...
void LeabraUnitSpec::Compute_Vm(LeabraUnit* u, LeabraLayer*, LeabraInhib*, LeabraTrial* trl) {
  u->I_net = 
...
void LeabraLayerSpec::Compute_Inhib_impl(LeabraLayer* lay, Unit_Group* ug, LeabraInhib* thr, LeabraTrial* trl) {
  if(compute_i == UNIT_INHIB) {
    ...
    if(compute_i == KWTA_INHIB)
      ...
      Compute_Inhib_AvgMaxPt(lay, ug, thr, trl);
...
void LeabraLayerSpec::Compute_Inhib_AvgMaxPt(LeabraLayer* lay, Unit_Group*, LeabraInhib* thr, LeabraTrial*) {
  float pt = i_kwta_pt;
  if(adapt_i.type == AdaptISpec::KWTA_PT)
    pt = thr->adapt_i.i_kwta_pt;
...
void LeabraLayerSpec::AdaptGBarI(LeabraLayer* lay, LeabraTrial*) {
...
void ScalarValLayerSpec::Compute_dWtUgp(Unit_Group* ugp, LeabraLayer* lay, LeabraTrial* trl) {
...
void ScalarValLayerSpec::Compute_dWt(LeabraLayer* lay, LeabraTrial* trl) {
  UNIT_GP_ITR(lay, 
...
bool TwoDValLayerSpec::CheckConfig(LeabraLayer* lay, LeabraTrial* trl, bool quiet) {
  if(!LeabraLayerSpec::CheckConfig(lay, trl, quiet)) return false;
...
void TwoDValLayerSpec::Compute_NetScale(LeabraLayer* lay, LeabraTrial* trl) {
  LeabraLayerSpec::Compute_NetScale(lay, trl);
  if(lay->hard_clamped) return;
  LeabraUnit* u;
  taLeafItr i;
...
void TwoDValLayerSpec::Compute_ActAvg_ugp(LeabraLayer* lay, Unit_Group* ug, LeabraInhib* thr, LeabraTrial*) {
  thr->acts.avg = 0.0f;
  thr->acts.max = -FLT_MAX;
  thr->acts.max_i = -1;
  LeabraUnit* u;
...
void TwoDValLayerSpec::Compute_ActMAvg_ugp(LeabraLayer* lay, Unit_Group* ug, LeabraInhib* thr, LeabraTrial*) {
  thr->acts_m.avg = 0.0f;
  thr->acts_m.max = -FLT_MAX;
  thr->acts_m.max_i = -1;
  LeabraUnit* u;
...
void TwoDValLayerSpec::Compute_ActPAvg_ugp(LeabraLayer* lay, Unit_Group* ug, LeabraInhib* thr, LeabraTrial*) {
  thr->acts_p.avg = 0.0f;
  thr->acts_p.max = -FLT_MAX;
  thr->acts_p.max_i = -1;
  LeabraUnit* u;
...
void TwoDValLayerSpec::ClampValue(Unit_Group* ugp, LeabraTrial*, float rescale) {
  if(ugp->size < 3) return;	// must be at least a few units..
  Layer* lay = ugp->own_lay;
...
void TwoDValLayerSpec::ReadValue(Unit_Group* ugp, LeabraTrial*) {
  if(ugp->size < 3) return;	// must be at least a few units..
  Layer* lay = ugp->own_lay;
  ...
  if(twod.n_vals == 1) {	// special case
    ...
    int outi = 0;  int j = 0;
    while(outi < twod.n_vals) {
      ValIdx& vi = sort_ary[sort_ary.size - j - 1];
...
void TwoDValLayerSpec::ResetAfterClamp(LeabraLayer* lay, LeabraTrial*) {
  UNIT_GP_ITR(lay, 
	      if(ugp->size > 2) {
...
void TwoDValLayerSpec::HardClampExt(LeabraLayer* lay, LeabraTrial* trl) {
  LeabraLayerSpec::Compute_HardClamp(lay, trl);
  ResetAfterClamp(lay, trl);
}

void TwoDValLayerSpec::Compute_HardClamp(LeabraLayer* lay, LeabraTrial* trl) {
  if(twod.clamp_pat) {
    LeabraLayerSpec::Compute_HardClamp(lay, trl);
    return;
  }
...
void TwoDValLayerSpec::Compute_Act_impl(LeabraLayer* lay, Unit_Group* ug, LeabraInhib* thr, LeabraTrial* trl) {
  LeabraLayerSpec::Compute_Act_impl(lay, ug, thr, trl);
  ReadValue(ug, trl);		// always read out the value
}

void TwoDValLayerSpec::Compute_dWtUgp(Unit_Group* ugp, LeabraLayer* lay, LeabraTrial* trl) {
  for(int i=lay->geom.x;i<ugp->size;i++) {
    LeabraUnit* u = (LeabraUnit*)ugp->FastEl(i);
    u->Compute_dWt(lay, trl);
  }
...
void TwoDValLayerSpec::Compute_dWt(LeabraLayer* lay, LeabraTrial* trl) {
  UNIT_GP_ITR(lay, 
	      Compute_dWtUgp(ugp, lay, trl);
	      );
  AdaptKWTAPt(lay, trl);
			       ...
			       LeabraTrial* trl, bool set_both)
{
  ...
  if((trl->phase == LeabraTrial::PLUS_PHASE) && (trl->phase_no < 2)) {
    ...
    if(da_mod.on && (da_mod.mod == DaModSpec::NEG_DIP)) {
      float new_neg = 0.0f;
      if(lu->dav < 0.0f) {
...
void LVLayerSpec::HelpConfig() {
  String help = "LVLayerSpec Local Value Computation:\n\
 Continuously learns about local external rewards, and cancels these reward values in DA computation.\
...
void PVLayerSpec::HelpConfig() {
  String help = "PVLayerSpec Perceived Value computation (fast and slow):\n\
 Learns perceived values (PV) according to the PVLV algorithm: looks at current network state\
 and computes how much it resembles states that have been associated with local value (LV) in the past\n\
...
void PVLayerSpec::Compute_PVPlusPhaseDwt(LeabraLayer* lay, LeabraTrial* trl) {
  ...
  UNIT_GP_ITR
    (lay, 
     ...
     if(pv.lrn_mode == PVSpec::ER) {
       if(er_avail) {
	 u->ext = (1.0f - pv.discount) * ext_rew_val;
	 ClampValue(ugp, trl); // apply new value
	 Compute_ExtToPlus(ugp, trl); // copy ext values to act_p
	 Compute_dWtUgp(ugp, lay, trl);
       }
...
void PVLVDaLayerSpec::Compute_Da(LeabraLayer* lay, LeabraTrial* trl) {
  ...
  FOR_ITR_EL(DaModUnit, u, lay->units., i) {
	...
	if(er_avail)
      ...
      else if(da.mode == PVLVDaSpec::PV_IF_ER_LV) {
	u->dav = pv_da;
	if(er_avail)
	  u->dav += lv_da;
      }
...
void LeabraWiz::TD(Network* net, bool bio_labels, bool td_mod_all) {
 ...
 at this time.  This also prevents other checks from being made.  All of this can be\
  ...
  bool ok = tdrpsp->CheckConfig(tdrp, trl, true) && tdintsp->CheckConfig(tdint, trl, true)
    && tdsp->CheckConfig(tdda, trl, true) && ersp->CheckConfig(extrew, trl, true);
...
void LeabraWiz::PVLV(Network* net, bool bio_labels, bool localist_val, bool fm_hid_cons, bool fm_out_cons, bool da_mod_all) {
 ...
 can be sure everything is ok.";
  ...
  bool	pvf_new = false;
  String lvnm = "NAcLV";  String pvfnm = "ABL";  String pvsnm = "NAcPV";  String vtanm = "VTA";
  String alt_lvnm = "LV";  String alt_pvfnm = "PVf";  String alt_pvsnm = "PVs"; String alt_vtanm = "DA";
  if(!bio_labels) {
    lvnm = "LV";    pvfnm = "PVf";    pvsnm = "PVs";    vtanm = "DA";
    alt_lvnm = "NAcLV"; alt_pvfnm = "ABL"; alt_pvsnm = "NAcPV"; alt_vtanm = "VTA";
  }

  bool dumbo;
  LeabraLayer* rew_targ_lay = (LeabraLayer*)net->FindMakeLayer("RewTarg");
  LeabraLayer* extrew = (LeabraLayer*)net->FindMakeLayer("ExtRew");
  LeabraLayer* lv = (LeabraLayer*)net->FindMakeLayer(lvnm, NULL, dumbo, alt_lvnm);
  LeabraLayer* pvf = (LeabraLayer*)net->FindMakeLayer(pvfnm, NULL, pvf_new, alt_pvfnm);
  LeabraLayer* pvs = (LeabraLayer*)net->FindMakeLayer(pvsnm, NULL, dumbo, alt_pvsnm);
  LeabraLayer* vta = (LeabraLayer*)net->FindMakeLayer(vtanm, NULL, dumbo, alt_vtanm);
  if(rew_targ_lay == NULL || pvf == NULL || extrew == NULL || lv == NULL || vta == NULL) return;

  //////////////////////////////////////////////////////////////////////////////////
  // sort layers

  rew_targ_lay->name = "0000";  extrew->name = "0001"; lv->name = "0002";  
  pvf->name = "0003";  pvs->name = "0004";    vta->name = "0005";

  net->layers.Sort();

  rew_targ_lay->name = "RewTarg";  extrew->name = "ExtRew"; lv->name = lvnm;
  pvf->name = pvfnm; pvs->name = pvsnm; vta->name = vtanm;
  ...
  for(i=0;i<net->layers.size;i++) {
    ...
    lay->SetUnitType(&TA_DaModUnit);
    // todo: add any new bg layer exclusions here!
    if(lay != rew_targ_lay && lay != pvf && lay != extrew && lay != lv && lay != pvs && lay != vta
       && !laysp->InheritsFrom(&TA_PFCLayerSpec) && !laysp->InheritsFrom(&TA_MatrixLayerSpec)
       && !laysp->InheritsFrom(&TA_PatchLayerSpec) 
       && !laysp->InheritsFrom(&TA_SNcLayerSpec) && !laysp->InheritsFrom(&TA_SNrThalLayerSpec)) {
      other_lays.Link(lay);
...
//     gpprfx = "DA_";
  ...
  PVConSpec* lv_cons = (PVConSpec*)learn_cons->FindMakeChild("LV", &TA_LVConSpec, dumbo, "NAc_LV");
  PVConSpec* pvf_cons = (PVConSpec*)learn_cons->FindMakeChild("PVf", &TA_PVConSpec, dumbo, "ABL_PV");
  PVConSpec* pvs_cons = (PVConSpec*)pvf_cons->FindMakeChild("PVs", &TA_PVConSpec, dumbo, "NAc_PVS");
  LeabraConSpec* bg_bias = (LeabraConSpec*)learn_cons->FindMakeChild("BgBias", &TA_LeabraBiasSpec);
  if(bg_bias == NULL) return;
  LeabraConSpec* fixed_bias = (LeabraConSpec*)bg_bias->FindMakeChild("FixedBias", &TA_LeabraBiasSpec);

  LeabraConSpec* marker_cons = (LeabraConSpec*)cons->FindMakeSpec("MarkerCons", &TA_MarkerConSpec);
  if(pvf_cons == NULL || marker_cons == NULL || fixed_bias == NULL)
    return;

  ExtRewLayerSpec* ersp = (ExtRewLayerSpec*)layers->FindMakeSpec("ExtRewLayer", &TA_ExtRewLayerSpec);
  LVLayerSpec* lvsp = (LVLayerSpec*)layers->FindMakeSpec(lvnm + "Layer", &TA_LVLayerSpec, dumbo, alt_lvnm + "Layer");
  PVLayerSpec* pvfsp = (PVLayerSpec*)layers->FindMakeSpec(pvfnm + "Layer", &TA_PVLayerSpec, dumbo, alt_pvfnm + "Layer");
  PVLayerSpec* pvssp = (PVLayerSpec*)pvfsp->FindMakeChild(pvsnm + "Layer", &TA_PVSLayerSpec, dumbo, alt_pvsnm + "Layer");
  PVLVDaLayerSpec* dasp = (PVLVDaLayerSpec*)layers->FindMakeSpec(vtanm + "Layer", &TA_PVLVDaLayerSpec, dumbo, alt_vtanm + "Layer");
  if(pvfsp == NULL || ersp == NULL || lvsp == NULL || dasp == NULL) return;
  ...
  lv_cons->SetUnique("lrate", true);
  pvf_cons->SetUnique("lrate", true);
  pvs_cons->SetUnique("lrate", true);

  // NOT unique: inherit from pvf:
  ...
  if(localist_val) {
    ersp->scalar.rep = ScalarValSpec::LOCALIST;
    lvsp->scalar.rep = ScalarValSpec::LOCALIST;
    pvfsp->scalar.rep = ScalarValSpec::LOCALIST;

    ersp->scalar.min_sum_act = .2f;
    lvsp->scalar.min_sum_act = .2f;
    pvfsp->scalar.min_sum_act = .2f;

    dasp->da.mode = PVLVDaSpec::PV_IF_ER_LV;

    pvfsp->bias_val.un = ScalarValBias::GC;
    pvfsp->bias_val.wt = ScalarValBias::NO_WT;
    pvfsp->bias_val.val = 0.0f;
    lvsp->bias_val.un = ScalarValBias::GC;
    lvsp->bias_val.wt = ScalarValBias::NO_WT;
    lvsp->bias_val.val = 0.5f;
    lvsp->lv.thr_min = .2f;
    lvsp->lv.thr_max = .8f;

    ersp->compute_i = LeabraLayerSpec::KWTA_AVG_INHIB; ersp->i_kwta_pt = 0.9f;
    lvsp->compute_i = LeabraLayerSpec::KWTA_AVG_INHIB; lvsp->i_kwta_pt = 0.9f;
    pvfsp->compute_i = LeabraLayerSpec::KWTA_AVG_INHIB; pvfsp->i_kwta_pt = 0.9f;

    ersp->kwta.k = 1; 	ersp->gp_kwta.k = 1; 
    lvsp->kwta.k = 1; 	lvsp->gp_kwta.k = 1; 
    pvfsp->kwta.k = 1; 	pvfsp->gp_kwta.k = 1; 

    ersp->unit_range.min = 0.0f;  ersp->unit_range.max = 1.0f;
    ersp->unit_range.UpdateAfterEdit();
    ersp->val_range = ersp->unit_range;

    lvsp->unit_range.min = 0.0f;  lvsp->unit_range.max = 1.0f;
    lvsp->unit_range.UpdateAfterEdit();
    lvsp->val_range = lvsp->unit_range;

    pvfsp->unit_range.min = 0.0f;  pvfsp->unit_range.max = 1.0f;
    pvfsp->unit_range.UpdateAfterEdit();
    pvfsp->val_range = pvfsp->unit_range;

    lv_units->SetUnique("act", true);
    lv_units->SetUnique("act_fun", true);
    lv_units->SetUnique("dt", true);
    lv_units->act_fun = LeabraUnitSpec::NOISY_LINEAR;
    lv_units->act.thr = .17f;
    lv_units->act.gain = 220.0f;
    lv_units->act.nvar = .01f;
    lv_units->g_bar.l = .1f;
    lv_units->g_bar.h = .03f;  lv_units->g_bar.a = .09f;
    lv_units->dt.vm = .05f;
    ...
    lv_cons->SetUnique("lmix", true);
    lv_cons->lmix.err_sb = false; // key for linear mapping w/out overshoot!
    lv_cons->SetUnique("rnd", true);
    lv_cons->rnd.mean = 0.1f;
    lv_cons->rnd.var = 0.0f;

    pvf_cons->SetUnique("lmix", true);
    pvf_cons->lmix.err_sb = false; // key for linear mapping w/out overshoot!
    pvf_cons->SetUnique("rnd", true);
    pvf_cons->rnd.mean = 0.1f;
    pvf_cons->rnd.var = 0.0f;
    ...
    ersp->scalar.rep = ScalarValSpec::GAUSSIAN;
    lvsp->scalar.rep = ScalarValSpec::GAUSSIAN;
    pvfsp->scalar.rep = ScalarValSpec::GAUSSIAN;
    ...
    ersp->compute_i = LeabraLayerSpec::KWTA_INHIB; ersp->i_kwta_pt = 0.25f;
    lvsp->compute_i = LeabraLayerSpec::KWTA_INHIB; lvsp->i_kwta_pt = 0.25f;
    pvfsp->compute_i = LeabraLayerSpec::KWTA_INHIB; pvfsp->i_kwta_pt = 0.25f;

    ersp->kwta.k = 3; 	ersp->gp_kwta.k = 3; 
    lvsp->kwta.k = 3; 	lvsp->gp_kwta.k = 3; 
    pvfsp->kwta.k = 3; 	pvfsp->gp_kwta.k = 3; 

    ersp->unit_range.min = -0.50f;  ersp->unit_range.max = 1.5f;
    ersp->unit_range.UpdateAfterEdit();

    lvsp->unit_range.min = -0.5f;  lvsp->unit_range.max = 1.5f;
    lvsp->unit_range.UpdateAfterEdit();

    pvfsp->unit_range.min = -0.5f;  pvfsp->unit_range.max = 1.5f;
    pvfsp->unit_range.UpdateAfterEdit();

    lv_units->SetUnique("act", true);
    lv_units->SetUnique("act_fun", true);
    lv_units->SetUnique("dt", true);
    lv_units->act_fun = LeabraUnitSpec::NOISY_XX1;
    lv_units->act.thr = .25f;
    lv_units->act.gain = 600.0f;
    lv_units->act.nvar = .005f;
    lv_units->g_bar.l = .1f;
    ...
    lv_cons->SetUnique("lmix", true);
    lv_cons->lmix.err_sb = true; // key for linear mapping w/out overshoot!
    lv_cons->SetUnique("rnd", true);
    lv_cons->rnd.mean = 0.1f;
    lv_cons->rnd.var = 0.0f;

    pvf_cons->SetUnique("lmix", true);
    pvf_cons->lmix.err_sb = true; // key for linear mapping w/out overshoot!
    pvf_cons->SetUnique("rnd", true);
    pvf_cons->rnd.mean = 0.1f;
    pvf_cons->rnd.var = 0.0f;
  ...
  if(lvsp->scalar.rep == ScalarValSpec::LOCALIST)
    n_pv_u = 4;
  else if(lvsp->scalar.rep == ScalarValSpec::GAUSSIAN)
    n_pv_u = 12;
  else
    n_pv_u = 21;
  ...
  if(pvf_new) {
    ...
    vta->pos.z = 0; vta->pos.y = 1; vta->pos.x = n_pv_u + 2;
    lv->pos.z = 0; lv->pos.y = 4; lv->pos.x = 0;
    pvs->pos.z = 0; pvs->pos.y = 2; pvs->pos.x = 0;
    pvf->pos.z = 0; pvf->pos.y = 0; pvf->pos.x = 0;
  }

  if(lv->n_units != n_pv_u) { lv->n_units = n_pv_u; lv->geom.x = n_pv_u; lv->geom.y = 1; }
  if(pvf->n_units != n_pv_u) { pvf->n_units = n_pv_u; pvf->geom.x = n_pv_u; pvf->geom.y = 1; }
  ...
  lv->SetLayerSpec(lvsp);	lv->SetUnitSpec(lv_units);
  pvf->SetLayerSpec(pvfsp);	pvf->SetUnitSpec(pv_units);
  ...
  // FindMakePrjn(Layer* recv, Layer* send,
  net->FindMakePrjn(extrew, rew_targ_lay, onetoone, marker_cons);
  net->FindMakePrjn(lv, extrew, onetoone, marker_cons);
  net->FindMakePrjn(pvf, lv, onetoone, marker_cons);
  net->FindMakePrjn(pvs, lv, onetoone, marker_cons);

  net->FindMakePrjn(vta, lv, onetoone, marker_cons);
  net->FindMakePrjn(vta, pvf, onetoone, marker_cons);
  net->FindMakePrjn(vta, pvs, onetoone, marker_cons);

  net->FindMakePrjn(lv,  vta, onetoone, marker_cons);
  net->FindMakePrjn(pvf,  vta, onetoone, marker_cons);
  net->FindMakePrjn(pvs,  vta, onetoone, marker_cons);

  if(pvf_new || fm_hid_cons || fm_out_cons) {
    for(i=0;i<input_lays.size;i++) {
      Layer* il = (Layer*)input_lays[i];
      net->FindMakePrjn(lv, il, fullprjn, lv_cons);
      net->FindMakePrjn(pvf, il, fullprjn, pvf_cons);
      net->FindMakePrjn(pvs, il, fullprjn, pvs_cons);
    }
    if(fm_hid_cons) {
      for(i=0;i<hidden_lays.size;i++) {
	Layer* hl = (Layer*)hidden_lays[i];
	net->FindMakePrjn(lv, hl, fullprjn, lv_cons);
	net->FindMakePrjn(pvf, hl, fullprjn, pvf_cons);
    ...
    if(fm_out_cons) {
      for(i=0;i<output_lays.size;i++) {
	Layer* ol = (Layer*)output_lays[i];
	net->FindMakePrjn(lv, ol, fullprjn, lv_cons);
	net->FindMakePrjn(pvf, ol, fullprjn, pvf_cons);
 ...
 at this time.  This also prevents other checks from being made.  All of this can be\
  ...
  bool ok = lvsp->CheckConfig(lv, trl, true) && pvfsp->CheckConfig(pvf, trl, true)
    && pvssp->CheckConfig(pvf, trl, true)
 ...
 you need to do manually:\n\n" + man_msg;
  ...
  lvsp->UpdateAfterEdit();
  pvfsp->UpdateAfterEdit();
  ...
  SelectEdit* edit = pdpMisc::FindSelectEdit(proj);
  if(edit != NULL) {
    lv_cons->SelectForEditNm("lrate", edit, "lv");
    pvf_cons->SelectForEditNm("lrate", edit, "pvf");
    pvs_cons->SelectForEditNm("lrate", edit, "pvs");
    //    pvf_cons->SelectForEditNm("syn_dep", edit, "pvf");
    ersp->SelectForEditNm("rew", edit, "extrew");
    pvfsp->SelectForEditNm("pv", edit, "pvf");
    lvsp->SelectForEditNm("lv", edit, "lv");
//     lvsp->SelectForEditNm("scalar", edit, "lv");
//     pvfsp->SelectForEditNm("scalar", edit, "pvf");
    lvsp->SelectForEditNm("bias_val", edit, "lv");
    pvfsp->SelectForEditNm("bias_val", edit, "pv");
...
void LeabraWiz::BgPFC(Network* net, bool bio_labels, bool localist_val, int n_stripes, bool fm_hid_cons, bool fm_out_cons, bool mat_fm_pfc_full, bool make_patch, bool nolrn_pfc, bool da_mod_all, bool lr_sched) {
  bool make_snc = true;
...
//   if(make_snr_thal || make_patch) make_snc = true;
 ...
 can be sure everything is ok.";
  ...
  LeabraLayer* lv = (LeabraLayer*)net->FindLayer(lvnm);
  LeabraLayer* pvf = (LeabraLayer*)net->FindLayer(pvfnm);
  LeabraLayer* pvs = (LeabraLayer*)net->FindLayer(pvsnm);
  ...
  for(i=0;i<net->layers.size;i++) {
    LeabraLayer* lay = (LeabraLayer*)net->layers[i];
    lay->SetUnitType(&TA_DaModUnit);
    if(lay != rew_targ_lay && lay != extrew && lay != lv && lay != pvf && lay != pvs && lay != vta
  ...
  LeabraConSpec* lv_cons = (LeabraConSpec*)learn_cons->FindMakeChild("LV", &TA_LVConSpec);
  LeabraConSpec* pvf_cons = (LeabraConSpec*)learn_cons->FindMakeChild("PVf", &TA_PVConSpec);
  LeabraConSpec* pvs_cons = (LeabraConSpec*)pvf_cons->FindMakeChild("PVs", &TA_PVConSpec);
  ...
  PVLayerSpec* pvfsp = (PVLayerSpec*)layers->FindMakeSpec(pvfnm + "Layer", &TA_PVLayerSpec);
  ...
  LeabraLayerSpec* patchsp = (LeabraLayerSpec*)pvfsp->FindMakeChild("PatchLayer", &TA_PatchLayerSpec);
  ...
  // FindMakePrjn(Layer* recv, Layer* send,
  if(make_snc) {
    net->FindMakePrjn(snc, lv, fullprjn, marker_cons);
    net->FindMakePrjn(snc, pvf, fullprjn, marker_cons);
  ...
  net->FindMakePrjn(lv, pfc, fullprjn, lv_cons);
  net->FindMakePrjn(pvf, pfc, fullprjn, pvf_cons);
  net->FindMakePrjn(pvs, pfc, fullprjn, pvs_cons);

  if(make_patch) {
    net->FindMakePrjn(patch, pfc, gponetoone, pvf_cons);
  }
  ...
  for(i=0;i<input_lays.size;i++) {
    ...
    if(make_patch && patch_new) {
      net->FindMakePrjn(patch, il, fullprjn, pvf_cons);
    }
  }
  for(i=0;i<hidden_lays.size;i++) {
    Layer* hl = (Layer*)hidden_lays[i];
	...
	net->FindMakePrjn(matrix, hl, fullprjn, matrix_cons);
      if(make_patch && patch_new) {
	net->FindMakePrjn(patch, hl, fullprjn, pvf_cons);
      }
    }
  }
  if(fm_out_cons) {
	...
	net->FindMakePrjn(matrix, ol, fullprjn, matrix_cons);
      if(make_patch && patch_new) {
	net->FindMakePrjn(patch, ol, fullprjn, pvf_cons);
      }
    }
  }
