...
void TBHungryBoxImpl::reallocate() {
  ...
  while(!done) {
    ...
    if(oldcount == n) {// we've been here before
      done = true;
      est_size = (int)n;
      continue;
    }
    ...
    if (i < (n)) {		// we have too many
      // more is ok, visible takes care of it 
      // no, we need to know how many there are so we can view the whole thing..
      s->ShowLess(s->feeder,(int)i,s); 
      est_size = (int) i;
    }
    else {
      if (span && (p > (bottom + span))) { // we have more room
	est_size = (int) n + (int) (( p-bottom) / span);
	s->ShowMore(s->feeder,est_size,s);
      }
      else {
	done = true;
	est_size = (int)n;
