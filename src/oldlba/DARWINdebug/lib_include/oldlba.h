...
#include <oldlba/oldlba_TA_type.h>
#include <math.h>
#include <limits.h>
#include <float.h>
...
inline void LeabraConSpec::UpdateWeights(Con_Group* cg, Unit* ru) {
  ...
  CON_GROUP_LOOP(cg, C_UpdateWeights((LeabraCon*)cg->Cn(i), lcg,
				     (LeabraUnit*)ru, (LeabraUnit*)cg->Un(i), lrus));
...
public:
  ...
  inline void 	C_Compute_dWt(LeabraCon* cn, LeabraUnit*, float heb, float err, float k_mult) {
    float dwt = lmix.err * err + lmix.hebb * heb;
    cn->dwt += cur_lrate * k_mult * dwt;
  }
...
public:
  ...
  inline void  C_Compute_dWt(LeabraCon* cn, LeabraUnit* ru,
...
inline void LeabraTabledConSpec::C_Compute_dWt(LeabraCon* cn, LeabraUnit* ru, 
					       LeabraUnit* su, float heb, float err)
{
...
public:
				  ...
				  float ru_act, float su_act)
  {
    ...
    return ru_act * (su_act * (cg->max_cor - swt_eff) - (1.0f - su_act) * swt_eff);
  }
...
class LeabraNewHebbConSpec : public LeabraConSpec {
  // Leabra connection spec with new version of hebbian learning rule
public:
  enum HebbFun {
    CPCA_ORIG,			// original version of CPCA with savg cor and intrinsic wt squashing
    CPCA_SIMPLE,		// a simpler version where the wt is assumed to be .5, and squashing happens afterwards, and no savg cor
    SIN_XY
  };

  HebbFun	hebb_fun;	// which hebbian function to use
  float		fixed_wt; 	// fixed weight value to use for simple cpca version
  float		sinxy_freq;	// frequency of the sine wave in sin_xy (in units of PI)
  float		subsu_thr;	// threshold below which sending unit activity is subtracted
  float		subsu_gain;	// gain of subtracting su term

  inline float C_Compute_Hebb_CPCAorg(LeabraCon* cn, LeabraCon_Group* cg,
			      float ru_act, float su_act) {
    return ru_act * (su_act * (cg->max_cor + cn->wt) +
		     (1.0f - su_act) * cn->wt);
  }

  inline float C_Compute_Hebb_CPCAsimp(LeabraCon* cn, LeabraCon_Group*,
			      float ru_act, float su_act) {
    float heb = ru_act * (su_act  - fixed_wt);
    if(heb > 0.0f)	heb *= (1.0f + cn->wt);
    else		heb *= -cn->wt;	
    return heb;
  }

  inline float C_Compute_Hebb_SinXY(LeabraCon* cn, LeabraCon_Group*,
				    float ru_act, float su_act)
  {
    const float PI = 3.14159265;
    float radius = sqrt(su_act * su_act + ru_act * ru_act);
    float sinval = sin(sinxy_freq * PI * radius);
    if(su_act < subsu_thr) { sinval -= subsu_gain * (subsu_thr - su_act); }
    float heb = ru_act * sinval;
    if(heb > 0.0f)	heb *= (1.0f + cn->wt);
    else		heb *= -cn->wt;	
    return heb;
  }
  ...
  inline float C_Compute_Hebb(LeabraCon* cn, LeabraCon_Group* cg,
			      float ru_act, float su_act)
  {
    if(hebb_fun == CPCA_SIMPLE) return C_Compute_Hebb_CPCAsimp(cn, cg, ru_act, su_act);
    if(hebb_fun == SIN_XY) 	return C_Compute_Hebb_SinXY(cn, cg, ru_act, su_act);
    return C_Compute_Hebb_CPCAorg(cn, cg, ru_act, su_act);
  }
  ...
  virtual void	GraphHebb(GraphLog* disp_log, float wt_val = .5f, float savg_act = .25f);
  // #BUTTON #NULL_OK graph the hebbian learning rule for given values of weight and sending average activity

  void        Defaults();
  void        Initialize();
  void        Destroy()               { };
  SIMPLE_COPY(LeabraNewHebbConSpec);
  COPY_FUNS(LeabraNewHebbConSpec, LeabraConSpec);
  TA_BASEFUNS(LeabraNewHebbConSpec);
};

class SigAmpSpec : public SigmoidSpec {
  // ##NO_TOKENS #INLINE #NO_UPDATE_AFTER Specifies a Sigmoid with amplitude: amp / [1 + exp(-(x - off) * gain)]
public:
  float		amp;		// amplitude

  float		Eval(float x)	
  { return amp * ((1.0 / (1.0f + expf(-ClipNet((x - off) * gain)))) - 0.5f); }

  virtual void	WriteToFile(ostream& strm) {
    strm << off << " " << gain << " " << amp;
  }
  virtual void	ReadFromFile(istream& strm) {
    strm >> off >> gain >> amp;
  }

  void 	Initialize()		{ amp = 1.0f; }
  void	Destroy()		{ };
  SIMPLE_COPY(SigAmpSpec);
  COPY_FUNS(SigAmpSpec, taBase);
  TA_BASEFUNS(SigAmpSpec);
};

class LeabraSigHebbConSpec : public LeabraConSpec {
  // Leabra connection spec with mixture-of-sigmoids hebbian learning rule
public:
  SigAmpSpec	pre0;		// presynaptic sigmoids
  SigAmpSpec	pre1;
  SigAmpSpec	pre2;
  SigAmpSpec	pre3;
  SigAmpSpec	pst0;		// postsynaptic sigmoids
  SigAmpSpec	pst1;
  SigAmpSpec	pst2;
  SigAmpSpec	pst3;
  SigAmpSpec	prepst0;	// pre * post
  SigAmpSpec	prepst1;
  SigAmpSpec	prepst2;
  SigAmpSpec	prepst3;
  SigAmpSpec	ppdist0;	// sqrt(pre * pre + post * post)
  SigAmpSpec	ppdist1;
  SigAmpSpec	ppdist2;
  SigAmpSpec	ppdist3;
  ...
  inline float C_Compute_Hebb(LeabraCon* cn, LeabraCon_Group*,
			      float pst, float pre)
  {
    float prepst = pre * pst;
    float ppdist = sqrtf(pre * pre + pst * pst);
    float heb =
      pre0.Eval(pre) + pre1.Eval(pre) + pre2.Eval(pre) + pre3.Eval(pre) + 
      pst0.Eval(pst) + pst1.Eval(pst) + pst2.Eval(pst) + pst3.Eval(pst) + 
      prepst0.Eval(prepst) + prepst1.Eval(prepst) + prepst2.Eval(prepst) + prepst3.Eval(prepst) + 
      ppdist0.Eval(ppdist) + ppdist1.Eval(ppdist) + ppdist2.Eval(ppdist) + ppdist3.Eval(ppdist);
  ...
  virtual void	WriteToFile(ostream& strm); // #BUTTON write sigmoid parameters to file
  virtual void	ReadFromFile(istream& strm); // #BUTTON read sigmoid parameters from file
  ...
  void        Destroy()               { };
  void	      InitLinks();
  SIMPLE_COPY(LeabraSigHebbConSpec);
  COPY_FUNS(LeabraSigHebbConSpec, LeabraConSpec);
  TA_BASEFUNS(LeabraSigHebbConSpec);
};
...
class DaModSpec : public taBase {
  // #INLINE ##NO_TOKENS #NO_UPDATE_AFTER specs for da-based learning from dynamic da modulation of plus-phase activations
public:
  bool		on;		// whether to actually modulate activations by da values
  bool		post;		// #CONDEDIT_ON_on:true #DEF_false if true, apply da modulation at end of settling, not during settling
  float		gain;		// #CONDEDIT_ON_on:true gain multiplier of da values
  bool		p_dwt;		// whether units learn based on prior activation states (affects Compute_dWt and UpdateWeights calls)
...
public:
  DaModSpec	da_mod;		// da modulation of plus phase activations (for da-based learning)
...
class TDRewPredLayerSpec : public ScalarValLayerSpec {
  // simulates ABL (amygdala): predicts rewards: minus phase = clamped prior expected reward V^(t), plus = settles on expectation of future reward V^(t+1)
public:
...
class TDRewIntegLayerSpec : public ScalarValLayerSpec {
  // simulates NAc, delta over phases = DA td-like signal: integrates perceived and external rewards
public:
...
//////////////////////////////////////////////////////////////////////////////////////////
//	Pavlovian (PVLV): Perceived Value from Learned Value Reward Learning System	//
//////////////////////////////////////////////////////////////////////////////////////////

// LV (NAc) learns value: learns when external rewards come on
// PV (ABL) perceives value: generalizes LV to stimulus inputs
// PVLVDa (VTA) computes DA signal as: (ER - LV) + (PV - baserate)
...
//////////////////////////////////////////////////////////
//	LV value learning: NAc Nucleus Accumbens	//
//////////////////////////////////////////////////////////
...
public:
  ...
  float		thr_min;	// #DEF_0.25 minimum threshold on LV reward expectation magnitude required for saying ER (.25 or .3 for use_pt, 0 for !use_pt and ExtRew norew = 0)
  float		thr_max;	// #DEF_0.75 maximum threshold on LV reward expectation magnitude required for saying ER (use .75 for ExtRew norew val = .5, else use .4 for norew = 0)
...
class LVLayerSpec : public ScalarValLayerSpec {
  // simulates NAc: learns when values arise
public:
  LVSpec	lv;	// learning value specs
...
public:
  enum	LearnMode {
    ER,				// use the external reward value directly (cheating because we shouldn't really know when reward was not delivered, but works better because of this..)
    DA_LV,			// use DA value from LV - ER ONLY: does not include the PV system's own contribution to the DA signal & prevents weird self-feedback effects
    DA				// use DA value direct from VTA: does include PV system's own contribution to DA, which can cause weird self-feedback effects
  };

  LearnMode	lrn_mode;	// #DEF_ER learning mode: what signal to use to train value?
...
class PVLayerSpec : public ScalarValLayerSpec {
  // learns to perceive value based on inputs that are associated with rewards: simulates ABL (amygdala) as a fast-learning PV system
public:
...
public:
  enum	DaMode {
    IF_ER_LV,			// if ext rew, da = er - nac_lv, else da = abl - nac_pv
    LV_PLUS_PV			// da = (ER - LV) + (PV_F - PV_S)
  };

  DaMode	mode;		// #DEF_IF_ER_LV how to compute da in the 1st plus phase;  minus phase is always ABL - BL; post-plus phases always PV(fast) - MAX(PV(slow), min_abl) (+add_lv..)
  float		tonic_da;	// #DEF_0 set a tonic 'dopamine' (DA) level (offset to add to da values)
  float		min_pv_s;	// #DEF_0.1 minimum PV_slow value, so that a low PV_fast (ABL) value (~0) makes a negative da: DA_pv = PV_fast - MAX(PV_slow, min_pv_s)
  bool		pp_lv;		// #DEF_false add ER - LV da to post-plus phases (second plus, etc)
...
class PVLVDaLayerSpec : public LeabraLayerSpec {
  // computes PVLV 'Da' signal: basic value is PV_fast - PV_slow, in plus_phase includes LV component (ER - LV), depending on da.mode
public:
...
class PatchLayerSpec : public PVLayerSpec {
  // simulates Patch as a cur PV layer
public:
...
class SNcLayerSpec : public PVLVDaLayerSpec {
  // computes PVLV 'Da' signal: if ExtRew: ExtRew - Cur; else Cur - Prev; modulated by Patch per-stripe Da signal.  Note, act will go negative!
public:
...
public:
  ...
  inline float C_Compute_Hebb(LeabraCon* cn, LeabraCon_Group* cg, DaModUnit* ru, DaModUnit* su) {
    ...
    if((learn_rule == MOTOR_DELTA) || (learn_rule == MOTOR_DELTA))
      rval = ru->act_p * (su->act_p * (cg->max_cor + cn->wt) + (1.0f - su->act_p) * cn->wt);
    else
      rval = ru->act_p2 * (su->act_p * (cg->max_cor + cn->wt) + (1.0f - su->act_p) * cn->wt);
...
class MatrixUnitSpec : public DaModUnitSpec {
  // basal ganglia matrix units: fire actions or WM updates. modulated by da signals from NAc/Vta, Patch/SNc
public:
...
public:
  bool		on;		// #DEF_true whether to use this function or not
  float		go_p;		// #CONDEDIT_ON_on:true #DEF_0.1 probability of actually firing a random Go when all the conditions have been met
  float		rgo_da;		// #DEF_20 strength of DA for activating Go (gc.h) and inhibiting NoGo (gc.a) when max_mnt is up or a random Go is active (applied during learning, after Go/NoGo already computed)
  ...
  float		prf_rgo_da;	// #DEF_1 strength of DA for activating Go (gc.h) and inhibiting NoGo (gc.a) on a purely random basis (prf_go_p)
...
public:
  float		abs_thr;	// #DEF_0 absolute threshold on per stripe avg_go_da value (-1..1) for triggering random GO (if stripe_agd < abs_min then Go p(go_p))
...
public:
  ...
  virtual void 	TD(Network* net, bool bio_labels = true, bool td_mod_all = false);
  ...
  virtual void 	PVLV(Network* net, bool bio_labels = true, bool fm_hid_cons=true, bool fm_out_cons=false, bool da_mod_all = false);
  // #MENU_BUTTON configure PVLV (pavlovian perceived value from learned value) learning layers in a network; bio_labels = use biologically-based labels for layers, else functional; da_mod_all = have da value modulate all the regular units in the network

  virtual void 	BgPFC(Network* net, int n_stripes=4, bool fm_hid_cons=false, bool fm_out_cons=false, bool mat_fm_pfc_full = false, bool make_patch=false, bool nolrn_pfc=false, bool da_mod_all = false, bool lr_sched = true);
