...
#include <css/css_iv.h>		// for cancelEdits
...
#include <ta/ta_base.h>		// for debugging alloc_list 
...
int cssProg::ReadLn() {
     ...
     (top->fin == top->fshell))
  {
    ...
    if(taMisc::iv_active && rl_event_hook == NULL) // might get set to null somewhere -- reset it
...
void cssProgSpace::Reset() {
  cssivSession::CancelProgEdits(this);
  Restart();
