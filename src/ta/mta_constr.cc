...
void TypeSpace_Declare_Types(TypeSpace* ths, ostream& strm, const String_PArray&) {
...
void TypeSpace_Declare_Instances(TypeSpace* ths, ostream& strm, const String_PArray& hv) {
...
void TypeSpace_Includes(TypeSpace* ths, ostream& strm, const String_PArray& hv,
			bool instances)
{
  strm << "#include <ta/typea.h>\n";
  if(!instances) {
    strm << "#include <ta/typea_constr.h>\n";
    if(mta->gen_css) {
      strm << "#include <css/basic_types.h>\n";
      strm << "#include <css/c_ptr_types.h>\n";
      strm << "#include <css/ta_css.h>\n";
    }
  }
...
void TypeDef_Generate_Instances(TypeDef* ths, ostream& strm) {
  ...
  if(ths->InheritsFrom(TA_taRegFun) && mta->gen_css
  ...
  if(mta->gen_css && !ths->HasOption("NO_CSS"))
    MethodSpace_Generate_Stubs(&(ths->methods), ths, strm);
}
...
void MethodDef_GenFunCall(TypeDef* ownr, MethodDef* md, ostream& strm, int act_argc) {
  ...
  if(md->type->ptr == 0) {
    if(md->type->DerivesFrom(TA_int) || md->type->DerivesFrom(TA_short) ||
      ...
      cmd = "cssReal((double)";
    else if(md->type->DerivesFrom(TA_taString) || md->type->DerivesFrom(TA_taSubString))
      cmd = "cssString(";
    else {
...
void TypeSpace_Generate_Init(TypeSpace* ths, ostream& strm, const String_PArray& ppfiles) {
  ...
  for(i=0; i<ppfiles.size; i++) {
    String tmp = ppfiles.FastEl(i).before("_TA_type.h");
    if(tmp.contains('/'))
      tmp = tmp.after('/', -1);
    strm << "  ta_Init_" << tmp << "();\n";
  }
