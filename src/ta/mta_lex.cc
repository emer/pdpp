...
int MTA::lex() {
  ...
  do {
	...
	continue;
      ...
      if(c != '\n')		// readword did not Getc() this char
	c = skipwhite_nocr();
      if(c == '\n')		// ignore #<lineno><RETURN> directives
	continue;
      else if(c != '\"') {	// "
	...
	state = Find_Item;
      ...
      String cur_fname_only = cur_fname;
      if(cur_fname_only.contains('/'))
	cur_fname_only = cur_fname_only.after('/', -1);
	...
	state = Skip_File;
      ...
      String fname_only = fname;
      if(fname_only.contains('/'))
	fname_only = fname_only.after('/', -1);
