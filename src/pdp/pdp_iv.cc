...
int taivSpecMember::BidForMember(MemberDef* md, TypeDef* td) {
...
bool taivSpecMember::NoCheckBox(taivDialog* dlg) {
  if((dlg == NULL) || (dlg->cur_base == NULL) || (dlg->typ == NULL))
    return true;

  if(dlg->typ->InheritsFrom(TA_BaseSpec_MGroup))
    return false;		// always use a check box for these..

  if(dlg->typ->InheritsFrom(TA_BaseSpec)) {
    BaseSpec* bs = (BaseSpec*)dlg->cur_base;
    if(bs->FindParent() != NULL)
      return false;		// owner has a parent spec, needs a box
    return true;		// no owner, no box..
  }
  else if(dlg->typ->InheritsFrom(TA_BaseSubSpec)) {
    BaseSubSpec* bs = (BaseSubSpec*)dlg->cur_base;
    if(bs->FindParent() != NULL)
      return false;		// owner has a parent spec, needs a box
    return true;		// no owner, no box..
  }
  else {
    TAPtr tap = (TAPtr)dlg->cur_base;
...
taivData* taivSpecMember::GetDataRep(ivInputHandler* ih, taivDialog* dlg, taivData* par) {
  taivData* rdat;
  if(sub_types != NULL)
  ...
  if(NoCheckBox(dlg)) {
    ...
    taivPlusToggle* pt = new taivPlusToggle(rdat, dlg, par);
    return pt;
  }
}

void taivSpecMember::GetImage(taivData* dat, void* base, ivWindow* win) {
  taivDialog* dlg = dat->dialog;
  if(NoCheckBox(dlg)) {
    if(sub_types != NULL)
      sub_types->GetImage(dat,base,win);
    else
      taivMember::GetImage(dat, base, win);
  }
  else {
...
void taivSpecMember::GetMbrValue(taivData* dat, void* base, bool& first_diff) {
  taivDialog* dlg = dat->dialog;
  if(NoCheckBox(dlg)) {
    if(sub_types != NULL)
      ((taivMember*)sub_types)->GetMbrValue(dat, base, first_diff);
    else
      taivMember::GetMbrValue(dat, base, first_diff);
    return;
  }
...
void taivSpecMember::CmpOrigVal(taivData* dat, void* base, bool& first_diff) {
  ...
  *taivMisc::record_script << "  ths->SetUnique(\"" << mbr->name << "\", "
			   << new_val << ");" << endl;
}
...
int taivProcess::BidForEdit(TypeDef* td){
  if(td->InheritsFrom(&TA_Process))
    return taivEdit::BidForEdit(td) +1;
  return 0;
}

int taivProcess::Edit(void* base, ivWindow* win, bool wait, bool readonly, const ivColor* bgclr) {
  ProcessDialog* dlg = (ProcessDialog*)taivMisc::FindEdit(base, typ);
  if(wait || (dlg == NULL) || dlg->CtrlPanel()) {
    dlg = new ProcessDialog(typ, base);
...
void taivProcess::Initialize()	{
  run_iv = NULL;
}

void taivProcess::Destroy() {
  if(run_iv) delete run_iv;
  run_iv = NULL;
}
...
int taivProcessRunBox::Edit(void* base, ivWindow* win, bool wait, bool readonly, const ivColor* bgclr) {
  ProcessDialog* dlg = (ProcessDialog*)taivMisc::FindEdit(base, typ);
  if(wait || (dlg == NULL) || !dlg->CtrlPanel()) {
    dlg = new Process_RunDlg(typ, base);
    dlg->Constr(win, wait, "", "", false, bgclr);
    dlg->cancel_only = readonly;
    return dlg->Edit();
  }
...
bool ProjObj_G::effect_select(bool set_select) {
  ...
  if(set_select) {
	...
	if((wb->iconified) || !wb->IsMapped()) {
	  wb->ViewWindow();
	  taivMisc::RecordScript(wb->GetPath() + "->ViewWindow();\n");
	}
	else {
	  wb->Iconify();
...
declare_taivMenuCallback(ProjEditor)
implement_taivMenuCallback(ProjEditor)
#include <ta/leave_iv.h>
...
ivGlyph* ProjEditor::GetLook() {
  ...
  buttonpatch = 
    new ivPatch(layout->vbox
		(layout->hbox(taivM->medium_button(min_but),
			      taivM->medium_button(max_but)),
		 layout->hcenter(layout->hflexible(toggle_spec_but),0),
		 wkit->menu_item_separator_look(),
		 layout->hbox(taivM->medium_button(actbutton[Tool::select]),
			      taivM->medium_button(actbutton[Tool::move])),
		 wkit->menu_item_separator_look(),
		 layout->hbox(taivM->medium_button(edit_but),
			      taivM->medium_button(init_but)),
		 layout->hbox(taivM->medium_button(icon_but),
			      taivM->medium_button(links_but)),
		 layout->hbox(taivM->medium_button(updt_but),
			      taivM->medium_button(agg_but)),
		 layout->hbox(taivM->medium_button(ctrl_but),
			      taivM->medium_button(proc_but)),
...
void ProjEditor::Iconify() {
  ...
  if(selectgroup.size == 0) { // iconify all
	...
	net->Iconify();
	taivMisc::RecordScript(net->GetPath() + "->Iconify();\n");
      }
      Environment* env;
      FOR_ITR_EL(Environment, env, owner->environments., i) {
	env->Iconify();
	taivMisc::RecordScript(env->GetPath() + "->Iconify();\n");
      }
      PDPLog* logo;
      FOR_ITR_EL(PDPLog, logo, owner->logs., i) {
	logo->Iconify();
	taivMisc::RecordScript(logo->GetPath() + "->Iconify();\n");
      }
    }
    InitDisplay();
  }
...
void ProjEditor::UpdateSelections() {
  ...
  if(pg.size > 0 && (ng.size + eg.size + lg.size > 0)) {
	...
	if(vw->updaters.FindEl(pr) >= 0) {
	  ...
	  taivMisc::RecordScript(vw->GetPath() + "->RemoveUpdater(" + pr->GetPath() + ");\n");
	}
	else {
	  DMEM_GUI_RUN_IF {
	    vw->AddUpdater(pr);
	  }
	  taivMisc::RecordScript(vw->GetPath() + "->AddUpdater(" + pr->GetPath() + ");\n");
	}
      }
      for(j=0;j<eg.size;j++) {
	...
	if(l->log_proc.FindEl(pr) >= 0) {
	  ...
	  taivMisc::RecordScript(l->GetPath() + "->RemoveUpdater(" + pr->GetPath() + ");\n");
	}
	else {
	  l->AddUpdater(pr);
	  if(taivMisc::record_script != NULL)
	    taivMisc::RecordScript(l->GetPath() + "->AddUpdater(" + pr->GetPath() + ");\n");
	}
      }
    }
  }
	...
	o->GetTypeDef()->name + " to:";
      TypeDef* newt = taivTypeDialog::GetType(td, prompt, o->GetTypeDef());
      if(newt != NULL) {
	String recordscript = o->GetPath() + "->ChangeMyType(" + newt->name + ");\n";
	DMEM_GUI_RUN_IF {
	  o->ChangeMyType(newt);
	}
	taivMisc::RecordScript(recordscript);
      }
    }
  }
  InitDisplay();
...
void ProjEditor::AggStats() {
  ...
  if(sg.size == 1) {	// one stat = set aggregator opr
    ...
    if(enm != NULL) {
      ...
      int nwval = taivEnumDialog::GetEnum(enm, prompt, sp->time_agg.op);
      if(nwval >= 0) {
	DMEM_GUI_RUN_IF {
	  sp->time_agg.op = (Aggregate::Operator)nwval;
	  sp->UpdateAfterEdit();
	}
	taivMisc::RecordScript(sp->GetPath() + "->time_agg.op = " + String(nwval) + ";\n");
      }
    }
  }
	...
	sp1->time_agg.SetFrom(sp2);
      ...
      taivMisc::RecordScript(sp1->GetPath() + "->time_agg.SetFrom(" + sp2->GetPath() + ");\n");
    }
    else if(chs == 1) {
      DMEM_GUI_RUN_IF {
	sp2->time_agg.SetFrom(sp1);
      }
      taivMisc::RecordScript(sp2->GetPath() + "->time_agg.SetFrom(" + sp1->GetPath() + ");\n");
    }
  }
...
void ProjEditor::SetAggType() {
  ...
  if(enm != NULL) {
	...
	taivMisc::RecordScript(ms->GetPath() + "->time_agg.op = " + String(nwval) + ";\n");
      }
    }
  }
  InitDisplay();
...
void ProjEditor::SetProcFlags() {
  ...
  if(pg.size == 0) {
	...
	taivMisc::RecordScript(ms->GetPath() + "->mod.flag = true;\n");
      }
      else if(chs == 1) {
	DMEM_GUI_RUN_IF {
	  ms->mod.flag = false;  ms->UpdateAfterEdit();
	}
	taivMisc::RecordScript(ms->GetPath() + "->mod.flag = false;\n");
      }
      else if(chs == 2) {
	DMEM_GUI_RUN_IF {
	  ms->log_stat = true;  ms->UpdateAfterEdit();
	}
	taivMisc::RecordScript(ms->GetPath() + "->log_stat = true;\n");
      }
      else if(chs == 3) {
	DMEM_GUI_RUN_IF {
	  ms->log_stat = false;  ms->UpdateAfterEdit();
	}
	taivMisc::RecordScript(ms->GetPath() + "->log_stat = false;\n");
      }
      else if(chs == 4) {
	DMEM_GUI_RUN_IF {
	  ms->loop_init = Stat::INIT_IN_LOOP;  ms->UpdateAfterEdit();
	}
	taivMisc::RecordScript(ms->GetPath() + "->loop_init = Stat::INIT_IN_LOOP;\n");
      }
      else if(chs == 5) {
	DMEM_GUI_RUN_IF {
	  ms->loop_init = Stat::INIT_START_ONLY;  ms->UpdateAfterEdit();
	}
	taivMisc::RecordScript(ms->GetPath() + "->loop_init = Stat::INIT_START_ONLY;\n");
      }
      else if(chs == 6) {
	DMEM_GUI_RUN_IF {
	  ms->loop_init = Stat::NO_INIT;  ms->UpdateAfterEdit();
	}
	taivMisc::RecordScript(ms->GetPath() + "->loop_init = Stat::NO_INIT;\n");
      }
    }
  }
  else {
	...
	taivMisc::RecordScript(mp->GetPath() + "->mod.flag = true;\n");
      }
      else if(chs == 1) {
	DMEM_GUI_RUN_IF {
	  mp->mod.flag = false;  mp->UpdateAfterEdit();
	}
	taivMisc::RecordScript(mp->GetPath() + "->mod.flag = false;\n");
      }
    }
  }
  InitDisplay();
...
void ProjEditor::NewProcs() {
  ...
  if((prg.size == 1) && (pg.size == 1)) { // transfer to group
    ...
    taivMisc::RecordScript(tg->GetPath() + "->Transfer(" + pr->GetPath() + ");\n");
    DMEM_GUI_RUN_IF {
      tg->Transfer(pr);
    }
    ...
    taivMisc::RecordScript(tg->GetPath() + "->Transfer(" + st->GetPath() + ");\n");
    DMEM_GUI_RUN_IF {
      tg->Transfer(st);
    }
    winbMisc::DelayedMenuUpdate(tg);
    ...
    if(gpch == 0) {
      taivMisc::RecordScript(sp->GetPath() + "->loop_stats.Transfer(" + st->GetPath() + ");\n");
      DMEM_GUI_RUN_IF {
	sp->loop_stats.Transfer(st);
      }
    }
    else if(gpch == 1) {
      taivMisc::RecordScript(sp->GetPath() + "->final_stats.Transfer(" + st->GetPath() + ");\n");
      DMEM_GUI_RUN_IF {
    ...
    if(gpch == 0) {
      taivMisc::RecordScript(sp->GetPath() + "->init_procs.Transfer(" + pr->GetPath() + ");\n");
      DMEM_GUI_RUN_IF {
	sp->init_procs.Transfer(pr);
      }
    }
    else if(gpch == 1) {
      taivMisc::RecordScript(sp->GetPath() + "->loop_procs.Transfer(" + pr->GetPath() + ");\n");
      DMEM_GUI_RUN_IF {
	sp->loop_procs.Transfer(pr);
      }
    }
    else if(gpch == 2) {
      taivMisc::RecordScript(sp->GetPath() + "->final_procs.Transfer(" + pr->GetPath() + ");\n");
      DMEM_GUI_RUN_IF {
    ...
    if(pr2 == pr1own) {		// transfering our parent into us, first transfer us into parent's group
      ...
      taivMisc::RecordScript(pr2own->GetPath() + ".Transfer(" + pr1->GetPath() + ");\n");
      DMEM_GUI_RUN_IF {
	pr2own->Transfer(pr1);
      }
    }
    taivMisc::RecordScript(pr1->children.GetPath() + ".Transfer(" + pr2->GetPath() + ");\n");
    DMEM_GUI_RUN_IF {
	...
	if(nwval >= 0) {
	  ...
	  taivMisc::RecordScript(sp->GetPath() + "->CreateAggregates(" + String(nwval) + ");\n");
	  winbMisc::DelayedMenuUpdate(sp);
	}
      }
    }
...
void ProjEditor::CtrlPanel() {
  ...
  for(i=0;i<pg.size;i++) {
    SchedProcess* sp = (SchedProcess*)pg.FastEl(i);
    sp->ControlPanel();
    taivMisc::RecordScript(sp->GetPath() + "->ControlPanel();\n");
  }
}
...
void ProjEditor::NewObjects() {
  ...
  if((pg.size > 0) && (ng.size + eg.size + lg.size > 0)) {	// set link to network, env or log
	...
	taivMisc::RecordScript(pr->GetPath() + "->network = " + n->GetPath() + ";\n");
	pr->UpdateAfterEdit();
      }
      for(j=0;j<eg.size;j++) {
	...
	taivMisc::RecordScript(pr->GetPath() + "->environment = " + e->GetPath() + ";\n");
	if(pr->InheritsFrom(&TA_MultiEnvProcess)) {
	  DMEM_GUI_RUN_IF {
	    ((MultiEnvProcess*)pr)->environments.LinkUnique(e);
	  }
	  taivMisc::RecordScript(pr->GetPath() + "->environments.LinkUnique(" + e->GetPath() + ");\n");
	}
	pr->UpdateAfterEdit();
      }
	...
	taivMisc::RecordScript(prog->GetPath() + "->LinkUnique(" + pr->GetPath() + ");\n");
      }
    }
  }
	...
	sp1->init_procs.LinkUnique(sp2);
      ...
      taivMisc::RecordScript(sp1->GetPath() + "->init_procs.LinkUnique(" + sp2->GetPath() + ");\n");
    }
    else if(gpch == 1) {
      DMEM_GUI_RUN_IF {
	sp1->loop_procs.LinkUnique(sp2);
      }
      taivMisc::RecordScript(sp1->GetPath() + "->loop_procs.LinkUnique(" + sp2->GetPath() + ");\n");
    }
    else if(gpch == 2) {
      DMEM_GUI_RUN_IF {
	sp1->final_procs.LinkUnique(sp2);
      }
      taivMisc::RecordScript(sp1->GetPath() + "->final_procs.LinkUnique(" + sp2->GetPath() + ");\n");
    }
  }
...
void ProjEditor::DupeSelections() {
  ...
  for(i=0; i<selectgroup.size; i++) {
    o = selectgroup.FastEl(i);
    taivMisc::RecordScript(o->GetPath() + "->DuplicateMe();\n");
    DMEM_GUI_RUN_IF {
      o->DuplicateMe();
    }
  }
...
void ProjEditor::RemoveSelections() {
  ...
  if((proc_gps.size > 0) || (stat_gps.size > 0)) {
	...
	Process* pr = (Process*)pg.FastEl(i);
	taivMisc::RecordScript(gp->GetPath() + "->RemoveEl(" + pr->GetPath() + ");\n");
	DMEM_GUI_RUN_IF {
	  gp->Remove(pr);
	}
      }
	...
	taivMisc::RecordScript(pr->GetPath() + "->network = NULL;\n");
      }
    }
    for(j=0;j<eg.size;j++) {
      Environment* e = (Environment*)eg.FastEl(j);
	...
	taivMisc::RecordScript(pr->GetPath() + "->environment = NULL;\n");
      }
      if(pr->InheritsFrom(&TA_MultiEnvProcess)) {
	taivMisc::RecordScript(pr->GetPath() + "->environments.RemoveEl(" + e->GetPath() + ");\n");
	DMEM_GUI_RUN_IF {
	...
	if(o->InheritsFrom(&TA_taGroup_impl)) {
	  ...
	  if(!gp->owner->InheritsFrom(&TA_taList_impl)) {
	    taivMisc::RecordScript(gp->GetPath() + "->Reset();\n");
	    DMEM_GUI_RUN_IF {
	      gp->Reset();
	    }
	...
	if(o->InheritsFrom(&TA_SchedProcess)) {
	  ...
	  if((prob != NULL) && (prob->spg != NULL) && (prob->spg->sproc != sp)) {
		...
		ownsp->init_procs.Remove(sp);
	      ...
	      taivMisc::RecordScript(ownsp->GetPath() + "->init_procs.RemoveEl(" + sp->GetPath() + ");\n");
	    }
	    if(ownsp->loop_procs.FindEl(sp) >= 0) {
	      DMEM_GUI_RUN_IF {
		ownsp->loop_procs.Remove(sp);
	      }
	      taivMisc::RecordScript(ownsp->GetPath() + "->loop_procs.RemoveEl(" + sp->GetPath() + ");\n");
	    }
	    if(ownsp->final_procs.FindEl(sp) >= 0) {
	      DMEM_GUI_RUN_IF {
		ownsp->final_procs.Remove(sp);
	      }
	      taivMisc::RecordScript(ownsp->GetPath() + "->final_procs.RemoveEl(" + sp->GetPath() + ");\n");
	    }
	    continue;
	  }
	}
	taivMisc::RecordScript(o->GetPath() + "->Close();\n");
	DMEM_GUI_RUN_IF {
	  og->Close_Child(o);
	}
...
void ProjEditor::NetSetSpec() {
  ...
  if(sp->InheritsFrom(TA_UnitSpec)) {
    taivMisc::RecordScript(nv->GetPath() + "->SetUnitSpec(" + sp->GetPath() + ");\n");
    DMEM_GUI_RUN_IF {
      nv->SetUnitSpec((UnitSpec*)sp);
    }
  }
  else if(sp->InheritsFrom(TA_ConSpec)) {
    taivMisc::RecordScript(nv->GetPath() + "->SetConSpec(" + sp->GetPath() + ");\n");
    DMEM_GUI_RUN_IF {
      nv->SetConSpec((ConSpec*)sp);
    }
  }
  else if(sp->InheritsFrom(TA_ProjectionSpec)) {
    taivMisc::RecordScript(nv->GetPath() + "->SetPrjnSpec(" + sp->GetPath() + ");\n");
    DMEM_GUI_RUN_IF {
      nv->SetPrjnSpec((ProjectionSpec*)sp);
    }
  }
  else if(sp->InheritsFrom(TA_LayerSpec)) {
    taivMisc::RecordScript(nv->GetPath() + "->SetLayerSpec(" + sp->GetPath() + ");\n");
    DMEM_GUI_RUN_IF {
