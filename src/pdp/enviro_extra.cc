...
void FreqEvent_MGroup::Initialize() {
  frequency = 1.0f;
  fenv = NULL;
}

void FreqEvent_MGroup::InitLinks() {
  Event_MGroup::InitLinks();
  taBase::Own(list, this);
  fenv = GET_MY_OWNER(FreqEnv);
}

void FreqEvent_MGroup::CutLinks() {
  fenv = NULL;
  Event_MGroup::CutLinks();
}

void FreqEvent_MGroup::Copy_(const FreqEvent_MGroup& cp) {
  frequency = cp.frequency;
  list = cp.list;
}

void FreqEvent_MGroup::InitEvents(Environment*) {
  list.Reset();
  if(fenv == NULL)	return;
  int cnt = 0;
  taLeafItr i;
...
void FreqEnv::Initialize() {
  ...
  events.gp.SetBaseType(&TA_FreqEvent_MGroup);
}
...
void FreqEnv::InitEvents() {
  ...
  if(freq_level == EVENT) {
    ...
    for(i=0; i<events.leaf_gp->size; i++) {
      ...
      if(evg->InheritsFrom(TA_FreqEvent_MGroup)) {
	FreqEvent_MGroup* fevg = (FreqEvent_MGroup*)evg;
...
void FreqTimeEvent_MGroup::Initialize() {
  frequency = 1.0f;
}

void FreqTimeEvent_MGroup::Copy_(const FreqTimeEvent_MGroup& cp) {
  frequency = cp.frequency;
}
...
void FreqTimeEnv::Initialize() {
  ...
  events.gp.SetBaseType(&TA_FreqTimeEvent_MGroup);
}
...
void FreqTimeEnv::InitEvents() {
  ...
  if(freq_level == EVENT) {
    ...
    for(i=0; i<events.leaf_gp->size; i++) {
      Event_MGroup* evg = (Event_MGroup*)events.leaf_gp->FastEl(i);
      if(evg->InheritsFrom(TA_FreqTimeEvent_MGroup)) {
	float freq = ((FreqTimeEvent_MGroup*)evg)->frequency;
	if(sample_type == RANDOM) {
	  int j;
	  for(j=0; j<n_sample; j++) {
	    if(Random::ZeroOne() <= freq)
...
void FromFileEnv::Initialize() {
  ...
  events_per_epc = 100;
  file_pos = 0;
  event_file.cmp = false;
  event_file.mode = taFile::NO_AUTO;
}

void FromFileEnv::InitLinks() {
  Environment::InitLinks();
  taBase::Own(event_file, this);
  if(!taMisc::iv_active)
    event_file.GetGetFile();
  else {
    ivWidgetKit* wkit = ivWidgetKit::instance();
    event_file.gf = new taivGetFile(".","*pat*",NULL,wkit->style(),false);
    taRefN::Ref(event_file.gf);
  }
  event_file.gf->select_only = true;	// just selecting a file name here
  event_file.UpdateGF();
}

void FromFileEnv::Copy_(const FromFileEnv& cp) {
  event_file = cp.event_file;
...
void FromFileEnv::InitEvents() {
  ...
  if(!event_file.IsOpen()) {
    istream* strm = event_file.OpenFile(event_file.fname, true);
    if(strm == NULL) return;
    file_pos = 0;
  }
...
void FromFileEnv::ReadEvent(Event* ev) {
  istream& strm = *(event_file.gf->istrm);
  while(true) {
    if(binary) {
      Pattern* pat;
      taLeafItr pi;
