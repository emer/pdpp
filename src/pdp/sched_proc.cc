...
void SchedProcess::Initialize() {
  ...
  displays.SetBaseType(&TA_PDPView);
...
void SchedProcess::Copy_(const SchedProcess& cp) {
  ...
  displays.BorrowUnique(cp.displays);	// don't replicate because we don't own them...
...
void SchedProcess::StepUp() {
  ...
  if((network != NULL) && (network->views.size > 0)) {
...
void SchedProcess::NotifyNetObjects() {
  ...
  for(i=taivMisc::active_edits.size-1; i>=0; i--) {
    taivEditDialog* dlg = taivMisc::active_edits.FastEl(i);
...
bool SchedProcess::HandleEvents() {
  ...
  int rval = cssivSession::RunPending();
  return (bool)rval;
}
...
void SchedProcess::ReplacePointersHook(TAPtr old) {
  ...
  winbMisc::DelayedMenuUpdate(prv_gp);
...
void SequenceEpoch::GenCntrLog(LogData* ld, bool gen) {
  ...
  if(gen) {
    if(cntr_items.size < 2)
      GetCntrDataItems();
    if((cur_event_gp != NULL) && (!cur_event_gp->name.empty()))
      ld->AddString(cntr_items.FastEl(1), cur_event_gp->name); 
    else
      ld->AddString(cntr_items.FastEl(1), "n/a");
  }
}
