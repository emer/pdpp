...
public:
  ...
  static void 	SaveRecoverFile(int err);
  ...
  static const ivColor* GetObjColor(Project* prj, TypeDef* typ);
...
// PDPMGroup will update the project upon changes to any objects
class PDPMGroup : public MenuGroup_impl {
public:
  void		UpdateMenu();

  void 	Initialize();
  void 	Destroy();
  TA_BASEFUNS(PDPMGroup);
};
...
class PosMGroup : public PDPMGroup {
public:
  TDCoord	pos;		// Position of Group

  void	Initialize() 		{ };
  void 	Destroy()		{ };
  void	InitLinks()		{ PDPMGroup::InitLinks(); taBase::Own(pos,this); }
  void  Copy_(const PosMGroup& cp)	{ pos = cp.pos; }
  void  Copy(const PosMGroup& cp)	{ PDPMGroup::Copy(cp); Copy_(cp); }
  TA_BASEFUNS(PosMGroup);
};

#define PosMGroup_of(T)							      \
class T ## _MGroup : public PosMGroup {					      \
public:									      \
  void	Initialize() 		{ SetBaseType(el_typ = &TA_ ## T); }	      \
  void 	Destroy()		{ };					      \
  TA_BASEFUNS(T ## _MGroup);						      \
}
