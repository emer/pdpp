...
#include <ta_misc/array_iv.h>

class ProcessDialog : public taivEditDialog {
  // #IGNORE
public:
  ProcessDialog(TypeDef* tp, void* base)	: taivEditDialog(tp,base) { };
  ProcessDialog()				{ };
  ~ProcessDialog();

  virtual bool	CtrlPanel()	{ return false; }
  void		Ok();		// stop the process when these are hit..
  void		Cancel();
};
...
class Process_RunDlg : public ProcessDialog {
  // #IGNORE 
public:
  Process_RunDlg(TypeDef* tp, void* base) : ProcessDialog(tp,base) {};
  Process_RunDlg() {};
  ~Process_RunDlg();
...
class taivProcess : public taivEdit {
  // adds control buttons to the edit window
public:  
  taivProcess*	run_iv;

  int		BidForEdit(TypeDef* td);
  int 		Edit(void* base, ivWindow* win, bool wait=false,bool readonly=false, const ivColor* bgclr = NULL);

  void Initialize();
  void Destroy();
  TAIV_EDIT_INSTANCE(taivProcess, taivEdit);
};


class taivProcessRunBox : public taivProcess {
  // just has the control buttons
public:  
  int		BidForEdit(TypeDef*) 	{ return 0; }
  int 		Edit(void* base, ivWindow* win, bool wait=false, bool readonly=false, const ivColor* bgclr = NULL);
  TAIV_EDIT_INSTANCE(taivProcessRunBox, taivProcess);
};


class taivSpecMember : public taivMember {
...
public:
  ...
  void		CmpOrigVal(taivData* dat, void* base, bool& first_diff);

  virtual bool	NoCheckBox(taivDialog* dlg); // deterimine if check box should be used

  TAIV_MEMBER_INSTANCE(taivSpecMember, taivMember);
};
