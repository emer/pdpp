...
protected:
   static Unit *unit_llist;             // dPDP - Head of l-list representing all units on the
                                        //        (both shadow and local) on the current processor
   static int total_unit_count;         // dPDP - Total # of units
   static int proc_rank;                // dPDP - the MPI rank of the current processor
   static int comm_size;                // dPDP - the size of the current MPI Communicator
   static float *actArray;              // dPDP - transmission of unit activations over MPI
                                        //        can be made significantly more efficient if
                                        //        all of the activations are in a contigous
                                        //        chunck of memory;

   int global_id;                       // dPDP - Unique integer value for the current unit

   Unit *llist_next;                    // dPDP - Next unit in l-list represention of all units
public:
  bool local;                          // dPDP - indicates if this object represents a local
                                       //        or a 'shadow' unit.
  ...
  float 	*act;		// activation; dPDP - changed this var. into a pointer
  float 	net;		// net input
  ...
  char		fill[40];
#endif
  Unit();                  // dPDP - maintains unit ids and unit l-list
  ...
  void 	Compute_dWt()		{ spec->Compute_dWt(this); }
   ...
   //        unit activations
   static void DistributeUnits(int arg_proc_rank, int arg_comm_size);     

   // dPDP - Synchronizes across all processors all shadow and local units
   static void DistributedSync();
   ...
   static taGroup_impl* ExtractLocalFromList(taGroup_impl *global_list);
   ...
   static int PartitionUnits(int &first_id, int &last_id, int proc_rank);
...
private:
   static Layer *layer_llist;           // dPDP - Head of l-list representing all layers on the
                                        //        on the current processor
   Layer *llist_next;                   // dPDP - Next layer in l-list represention of all of
                                        //        the layers

public:
  taGroup_impl		units;		// units or groups of units
  taGroup_impl          *local_units;    // dPDP - all of the units in this layer that are
                                        //        local to the current processor
  ...
  static void InitLocalUnitGroups(); // dPDP - initialize local_units in ALL layer objects
